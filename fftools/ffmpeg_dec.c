...
static int send_frame_to_filters(InputStream *ist, AVFrame *decoded_frame)
{
    ...
    for (i = 0; i < ist->nb_filters; i++)
    {
        ...
        if (ret == AVERROR_EOF) ret = 0; /* ignore */
        if (ret < 0)
        {
...
static AVRational audio_samplerate_update(void *logctx, Decoder *d, const AVFrame *frame)
{
    ...
    if (frame->sample_rate == d->last_frame_sample_rate) goto finish;
    ...
    if (frame->time_base.num == 1 && frame->time_base.den > tb_new.den && !(frame->time_base.den % tb_new.den)) tb_new = frame->time_base;

    if (d->last_frame_pts != AV_NOPTS_VALUE) d->last_frame_pts = av_rescale_q(d->last_frame_pts, d->last_frame_tb, tb_new);
    d->last_frame_duration_est = av_rescale_q(d->last_frame_duration_est, d->last_frame_tb, tb_new);
...
static void audio_ts_process(void *logctx, Decoder *d, AVFrame *frame)
{
    ...
    pts_pred = d->last_frame_pts == AV_NOPTS_VALUE ? 0 : d->last_frame_pts + d->last_frame_duration_est;
    ...
    else if (d->last_frame_pts != AV_NOPTS_VALUE && frame->pts > av_rescale_q_rnd(pts_pred, tb, frame->time_base, AV_ROUND_UP))
    {
        // there was a gap in timestamps, reset conversion state
        d->last_filter_in_rescale_delta = AV_NOPTS_VALUE;
    }

    frame->pts = av_rescale_delta(frame->time_base, frame->pts, tb, frame->nb_samples, &d->last_filter_in_rescale_delta, tb);
...
static int64_t video_duration_estimate(const InputStream *ist, const AVFrame *frame)
{
    ...
    if (frame->duration > 0 && (!ifile->format_nots || ist->framerate.num)) return frame->duration;
    ...
    if (codec_duration > 0 && ifile->format_nots) return codec_duration;

    // when timestamps are available, repeat last frame's actual duration
    // (i.e. pts difference between this and last frame)
    if (frame->pts != AV_NOPTS_VALUE && d->last_frame_pts != AV_NOPTS_VALUE && frame->pts > d->last_frame_pts) return frame->pts - d->last_frame_pts;

    // try frame/codec duration
    if (frame->duration > 0) return frame->duration;
    if (codec_duration > 0) return codec_duration;

    // try average framerate
    if (ist->st->avg_frame_rate.num && ist->st->avg_frame_rate.den)
    {
        int64_t d = av_rescale_q(1, av_inv_q(ist->st->avg_frame_rate), frame->time_base);
        if (d > 0) return d;
    }
...
static int video_frame_process(InputStream *ist, AVFrame *frame)
{
    ...
    if (ist->dec_ctx->width != frame->width || ist->dec_ctx->height != frame->height || ist->dec_ctx->pix_fmt != frame->format)
    {
    ...
    if (frame->pts == AV_NOPTS_VALUE) frame->pts = d->last_frame_pts == AV_NOPTS_VALUE ? 0 : d->last_frame_pts + d->last_frame_duration_est;
    ...
    if (debug_ts)
    {
        av_log(ist, AV_LOG_INFO,
            ...
            av_ts2str(frame->pts), av_ts2timestr(frame->pts, &frame->time_base), av_ts2str(frame->pkt_dts), av_ts2timestr(frame->pkt_dts, &frame->time_base),
            av_ts2str(frame->duration), av_ts2timestr(frame->duration, &frame->time_base), !!(frame->flags & AV_FRAME_FLAG_KEY), frame->pict_type,
            frame->time_base.num, frame->time_base.den);
    }

    if (ist->st->sample_aspect_ratio.num) frame->sample_aspect_ratio = ist->st->sample_aspect_ratio;
...
static void sub2video_flush(InputStream *ist)
{
    for (int i = 0; i < ist->nb_filters; i++)
    {
        ...
        if (ret != AVERROR_EOF && ret < 0) av_log(NULL, AV_LOG_WARNING, "Flush the frame error.\n");
    }
}
...
static int process_subtitle(InputStream *ist, AVFrame *frame)
{
    ...
    {
        AVSubtitle *sub_prev = d->sub_prev[0]->buf[0] ? (AVSubtitle *)d->sub_prev[0]->buf[0]->data : NULL;
        int end = 1;
        if (sub_prev)
        {
            ...
            {
                av_log(NULL, AV_LOG_DEBUG, "Subtitle duration reduced from %" PRId32 " to %d%s\n", sub_prev->end_display_time, end,
                    end <= 0 ? ", dropping it" : "");
                sub_prev->end_display_time = end;
            }
        }
    ...
    if (!subtitle->num_rects) return 0;

    for (int oidx = 0; oidx < ist->nb_outputs; oidx++)
    {
        OutputStream *ost = ist->outputs[oidx];
        if (!ost->enc || ost->type != AVMEDIA_TYPE_SUBTITLE) continue;
...
int fix_sub_duration_heartbeat(InputStream *ist, int64_t signal_pts)
{
    Decoder *d = ist->decoder;
    int ret = AVERROR_BUG;
    AVSubtitle *prev_subtitle = d->sub_prev[0]->buf[0] ? (AVSubtitle *)d->sub_prev[0]->buf[0]->data : NULL;
    AVSubtitle *subtitle;

    if (!ist->fix_sub_duration || !prev_subtitle || !prev_subtitle->num_rects || signal_pts <= prev_subtitle->pts) return 0;
...
static int transcode_subtitles(InputStream *ist, const AVPacket *pkt, AVFrame *frame)
{
    ...
    if (!pkt)
    {
        ...
        if (!flush_pkt) return AVERROR(ENOMEM);
    }
    ...
    if (!got_output) return pkt ? 0 : AVERROR_EOF;
...
static int send_filter_eof(InputStream *ist)
{
    ...
    for (i = 0; i < ist->nb_filters; i++)
    {
        int64_t end_pts = d->last_frame_pts == AV_NOPTS_VALUE ? AV_NOPTS_VALUE : d->last_frame_pts + d->last_frame_duration_est;
...
static int packet_decode(InputStream *ist, AVPacket *pkt, AVFrame *frame)
{
    ...
    if (dec->codec_type == AVMEDIA_TYPE_SUBTITLE) return transcode_subtitles(ist, pkt, frame);
    ...
    while (1)
    {
        ...
        update_benchmark("decode_%s %d.%d", type_desc, ist->file_index, ist->index);
        ...
        {
            av_log(ist, exit_on_error ? AV_LOG_FATAL : AV_LOG_WARNING, "corrupt decoded frame\n");
            if (exit_on_error) return AVERROR_INVALIDDATA;
        }
...
{
    char name[16];
    snprintf(name, sizeof(name), "dec%d:%d:%s", ist->file_index, ist->index, ist->dec_ctx->codec->name);
...
static void *decoder_thread(void *arg)
{
    ...
    while (!input_status)
    {
        ...
        if (!dt.pkt->buf) av_log(ist, AV_LOG_VERBOSE, "Decoder thread received %s packet\n", flush_buffers ? "flush" : "EOF");
        ...
        if (ret == AVERROR_EOF)
        {
            ...
            if (ist->dec->type == AVMEDIA_TYPE_AUDIO)
            {
                ...
                dur.duration = av_rescale_q(ist->nb_samples, (AVRational){ 1, ist->dec_ctx->sample_rate }, ist->st->time_base);
        ...
        if (ret < 0)
        {
            if (ret != AVERROR_EOF) av_log(ist, AV_LOG_ERROR, "Error communicating with the main thread\n");
            break;
        }
    }
...
finish:
    ...
    if (ifile->audio_duration_queue && ist->dec->type == AVMEDIA_TYPE_AUDIO) av_thread_message_queue_set_err_recv(ifile->audio_duration_queue, AVERROR_EOF);
...
int dec_packet(InputStream *ist, const AVPacket *pkt, int no_eof)
{
    ...
    if (!d->queue_in) return AVERROR_EOF;
...
static int hw_device_setup_for_decode(InputStream *ist)
{
    ...
    else
    {
        ...
        else if (ist->hwaccel_id == HWACCEL_GENERIC)
        {
            ...
            // device.
            if (!dev && type == AV_HWDEVICE_TYPE_QSV) dev = hw_device_get_by_name("__qsv_device");

            if (!dev) err = hw_device_init_from_type(type, NULL, &dev);
        }
        else
        {
    ...
    if (!ist->dec_ctx->hw_device_ctx) return AVERROR(ENOMEM);
...
int dec_open(InputStream *ist)
{
    ...
    if (codec->type == AVMEDIA_TYPE_SUBTITLE && ist->fix_sub_duration)
    {
        for (int i = 0; i < FF_ARRAY_ELEMS(d->sub_prev); i++)
        {
            ...
            if (!d->sub_prev[i]) return AVERROR(ENOMEM);
        }
        d->sub_heartbeat = av_frame_alloc();
        if (!d->sub_heartbeat) return AVERROR(ENOMEM);
    }
    ...
    if (!av_dict_get(ist->decoder_opts, "threads", NULL, 0)) av_dict_set(&ist->decoder_opts, "threads", "auto", 0);
    /* Attached pics are sparse, therefore we would not want to delay their decoding till EOF. */
    if (ist->st->disposition & AV_DISPOSITION_ATTACHED_PIC) av_dict_set(&ist->decoder_opts, "threads", "1", 0);

    ret = hw_device_setup_for_decode(ist);
    if (ret < 0)
    {
        av_log(ist, AV_LOG_ERROR, "Hardware device setup failed for decoder: %s\n", av_err2str(ret));
        return ret;
    }
