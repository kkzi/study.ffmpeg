...
static void tb_update(const SyncQueue *sq, SyncQueueStream *st, const SyncQueueFrame frame)
{
    ...
    if (tb.num == st->tb.num && tb.den == st->tb.den) return;

    // timebase should not change after the first frame
    av_assert0(!av_fifo_can_read(st->fifo));

    if (st->head_ts != AV_NOPTS_VALUE) st->head_ts = av_rescale_q(st->head_ts, st->tb, tb);
...
static void finish_stream(SyncQueue *sq, unsigned int stream_idx)
{
    ...
    if (!st->finished) av_log(sq->logctx, AV_LOG_DEBUG, "sq: finish %u; head ts %s\n", stream_idx, av_ts2timestr(st->head_ts, &st->tb));
    ...
    if (st->limiting && st->head_ts != AV_NOPTS_VALUE)
    {
        ...
        for (unsigned int i = 0; i < sq->nb_streams; i++)
        {
            ...
            {
                if (!st1->finished) av_log(sq->logctx, AV_LOG_DEBUG, "sq: finish secondary %u; head ts %s\n", i, av_ts2timestr(st1->head_ts, &st1->tb));
    ...
    for (unsigned int i = 0; i < sq->nb_streams; i++)
    {
        if (!sq->streams[i].finished) return;
    }
    sq->finished = 1;
...
static void queue_head_update(SyncQueue *sq)
{
    ...
    if (sq->head_stream < 0)
    {
        ...
        for (unsigned int i = 0; i < sq->nb_streams; i++)
        {
            ...
            if (!st->limiting) continue;
            if (st->head_ts == AV_NOPTS_VALUE) return;
            if (first_limiting == UINT_MAX) first_limiting = i;
        }
...
static void stream_update_ts(SyncQueue *sq, unsigned int stream_idx, int64_t ts)
{
    ...
    if (ts == AV_NOPTS_VALUE || (st->head_ts != AV_NOPTS_VALUE && st->head_ts >= ts)) return;
    ...
    if (st->limiting && (sq->head_stream < 0 || sq->head_stream == stream_idx)) queue_head_update(sq);
}
...
static int overflow_heartbeat(SyncQueue *sq, int stream_idx)
{
    ...
    if (tail_ts == AV_NOPTS_VALUE || tail_ts >= st->head_ts || av_rescale_q(st->head_ts - tail_ts, st->tb, AV_TIME_BASE_Q) < sq->buf_size_us) return 0;
    ...
    for (unsigned int i = 0; i < sq->nb_streams; i++)
    {
        ...
        if (st == st1 || st1->finished || (st1->head_ts != AV_NOPTS_VALUE && av_compare_ts(tail_ts, st->tb, st1->head_ts, st1->tb) <= 0)) continue;

        ts = av_rescale_q(tail_ts, st->tb, st1->tb);
        if (st1->head_ts != AV_NOPTS_VALUE) ts = FFMAX(st1->head_ts + 1, ts);
...
int sq_send(SyncQueue *sq, unsigned int stream_idx, SyncQueueFrame frame)
{
    ...
    if (st->finished) return AVERROR_EOF;
...
static void offset_audio(AVFrame *f, int nb_samples)
{
    ...
    for (int i = 0; i < planes; i++)
    {
        ...
        if (i < FF_ARRAY_ELEMS(f->data)) f->data[i] = f->extended_data[i];
    }
    f->linesize[0] -= offset;
...
static int frame_is_aligned(const SyncQueue *sq, const AVFrame *frame)
{
    ...
    if (!((uintptr_t)frame->data[0] & sq->align_mask) && !(frame->linesize[0] & sq->align_mask) && frame->linesize[0] > sq->align_mask) return 1;
...
{
    SyncQueueStream *st_head = sq->head_stream >= 0 ? &sq->streams[sq->head_stream] : NULL;
    SyncQueueStream *st;
    ...
    if (av_fifo_can_read(st->fifo) && (st->frame_samples <= st->samples_queued || st->finished))
    {
        ...
        if (ts != AV_NOPTS_VALUE && st_head) cmp = av_compare_ts(ts, st->tb, st_head->head_ts, st_head->tb);
...
int sq_receive(SyncQueue *sq, int stream_idx, SyncQueueFrame frame)
{
     ...
     * for lagging streams */
    if (ret == AVERROR(EAGAIN) && overflow_heartbeat(sq, stream_idx)) ret = receive_internal(sq, stream_idx, frame);
...
int sq_add_stream(SyncQueue *sq, int limiting)
{
    ...
    if (!st->fifo) return AVERROR(ENOMEM);
...
void sq_limit_frames(SyncQueue *sq, unsigned int stream_idx, uint64_t frames)
{
    ...
    if (st->frames_sent >= st->frames_max) finish_stream(sq, stream_idx);
}
