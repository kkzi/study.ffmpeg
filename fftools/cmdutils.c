...
void show_help_options(const OptionDef *options, const char *msg, int req_flags, int rej_flags, int alt_flags)
{
    ...
    for (po = options; po->name; po++)
    {
        ...
        if (((po->flags & req_flags) != req_flags) || (alt_flags && !(po->flags & alt_flags)) || (po->flags & rej_flags)) continue;
...
static const OptionDef *find_option(const OptionDef *po, const char *name)
{
    ...
    {
        const char *end;
        if (av_strstart(name, po->name, &end) && (!*end || *end == ':')) break;
        po++;
    }
    return po;
}
...
static int write_option(void *optctx, const OptionDef *po, const char *opt, const char *arg)
{
    ...
    if (po->flags & OPT_EXIT) return AVERROR_EXIT;
...
int parse_options(void *optctx, int argc, char **argv, const OptionDef *options, int (*parse_arg_function)(void *, const char *))
{
    ...
    while (optindex < argc)
    {
        ...
        if (handleoptions && opt[0] == '-' && opt[1] != '\0')
        {
            ...
            if ((ret = parse_option(optctx, opt, argv[optindex], options)) < 0) return ret;
            optindex += ret;
        }
        else
        {
...
int locate_option(int argc, char **argv, const OptionDef *options, const char *optname)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if ((!po->name && !strcmp(cur_opt, optname)) || (po->name && !strcmp(optname, po->name))) return i;
...
static void dump_argument(FILE *report_file, const char *a)
{
    ...
    for (p = a; *p; p++)
        if (!((*p >= '+' && *p <= ':') || (*p >= '@' && *p <= 'Z') || *p == '_' || (*p >= 'a' && *p <= 'z'))) break;
    if (!*p)
    {
        fputs(a, report_file);
        return;
...
int opt_default(void *optctx, const char *opt, const char *arg)
{
    ...
    if ((o = opt_find(&fc, opt, NULL, 0, AV_OPT_SEARCH_CHILDREN | AV_OPT_SEARCH_FAKE_OBJ)))
    {
        ...
        if (consumed) av_log(NULL, AV_LOG_VERBOSE, "Routing option %s to both codec and muxer layer\n", opt);
        consumed = 1;
    }
#if CONFIG_SWSCALE
...
static int init_parse_context(OptionParseContext *octx, const OptionGroupDef *groups, int nb_groups)
{
    ...
    if (!octx->groups) return AVERROR(ENOMEM);
...
#define GET_ARG(arg)                                                                                                                                           \
    ...
    {                                                                                                                                                          \
        ...
        if (opt[0] == 'n' && opt[1] == 'o' && (po = find_option(options, opt + 2)) && po->name && po->flags & OPT_BOOL)
        {
...
FILE *get_preset_file(char *filename, size_t filename_size, const char *preset_name, int is_path, const char *codec_name)
{
    ...
    else
    {
        ...
        if (wchartoutf8(datadir_w, &datadir)) datadir = NULL;
        av_free(datadir_w);
...
int setup_find_stream_info_opts(AVFormatContext *s, AVDictionary *codec_opts, AVDictionary ***dst)
{
    ...
    *dst = NULL;
    ...
    if (!s->nb_streams) return 0;

    opts = av_calloc(s->nb_streams, sizeof(*opts));
    if (!opts) return AVERROR(ENOMEM);
...
void *allocate_array_elem(void *ptr, size_t elem_size, int *nb_elems)
{
    ...
    if (!(new_elem = av_mallocz(elem_size)) || av_dynarray_add_nofree(ptr, nb_elems, new_elem) < 0) return NULL;
    return new_elem;
}
...
{
    double theta = 0;
    if (displaymatrix) theta = -round(av_display_rotation_get((int32_t *)displaymatrix));
