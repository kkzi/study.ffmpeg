...
static void sub2video_push_ref(InputFilterPriv *ifp, int64_t pts)
{
    ...
    ret = av_buffersrc_add_frame_flags(ifp->filter, frame, AV_BUFFERSRC_FLAG_KEEP_REF | AV_BUFFERSRC_FLAG_PUSH);
    if (ret != AVERROR_EOF && ret < 0) av_log(NULL, AV_LOG_WARNING, "Error while add the frame to buffer source(%s).\n", av_err2str(ret));
}
...
static void sub2video_update(InputFilterPriv *ifp, int64_t heartbeat_pts, const AVSubtitle *sub)
{
    ...
    if (sub)
    {
        pts = av_rescale_q(sub->pts + sub->start_display_time * 1000LL, AV_TIME_BASE_Q, ifp->time_base);
        end_pts = av_rescale_q(sub->pts + sub->end_display_time * 1000LL, AV_TIME_BASE_Q, ifp->time_base);
...
static int choose_pix_fmts(OutputFilter *ofilter, AVBPrint *bprint, const char **dst)
{
    ...
    *dst = NULL;
    ...
    {
        *dst = ofp->format == AV_PIX_FMT_NONE ? NULL : av_get_pix_fmt_name(ofp->format);
    }
    else if (ofp->formats)
    {
        ...
        if (!av_bprint_is_complete(bprint)) return AVERROR(ENOMEM);
...
#define DEF_CHOOSE_FORMAT(name, type, var, supported_list, none, printf_format, get_name)                                                                      \
    ...
    {                                                                                                                                                          \
        if (ofp->var == none && !ofp->supported_list) return;                                                                                                  \
...
static int filter_opt_apply(AVFilterContext *f, const char *key, const char *val)
{
    ...
    if (ret == AVERROR_OPTION_NOT_FOUND && key[0] == '/') o = av_opt_find(f, key + 1, NULL, 0, AV_OPT_SEARCH_CHILDREN);
    if (!o) goto err_apply;
    ...
    if (ret < 0) goto err_apply;

    return 0;

err_apply:
    av_log(NULL, AV_LOG_ERROR, "Error applying option '%s' to filter '%s': %s\n", key, f->filter->name, av_err2str(ret));
    return ret;
err_load:
    av_log(NULL, AV_LOG_ERROR, "Error loading value for option '%s' from file '%s'\n", key, val);
    return ret;
}
    ...
    *outputs = NULL;
    ...
    if (hw_device)
    {
        for (int i = 0; i < graph->nb_filters; i++)
        {
            ...
            if (!(f->filter->flags & AVFILTER_FLAG_HWDEVICE)) continue;
...
static char *describe_filter_link(FilterGraph *fg, AVFilterInOut *inout, int in)
{
    ...
    if (nb_pads > 1) return av_strdup(ctx->filter->name);
    return av_asprintf("%s:%s", ctx->filter->name, avfilter_pad_get_name(pads, inout->pad_idx));
}
...
static int ifilter_bind_ist(InputFilter *ifilter, InputStream *ist)
{
    ...
    if (ifp->type_src == AVMEDIA_TYPE_SUBTITLE)
    {
        ...
        if (!ifp->sub2video.frame) return AVERROR(ENOMEM);
    }
...
static int set_channel_layout(OutputFilterPriv *f, OutputStream *ost)
{
    ...
    {
        if (c->ch_layouts[i].nb_channels == ost->enc_ctx->ch_layout.nb_channels) break;
    }
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        else
        {
            ...
            if (!strcmp(c->name, "mjpeg"))
            {
                ...
                static const enum AVPixelFormat mjpeg_formats[] = { AV_PIX_FMT_YUVJ420P, AV_PIX_FMT_YUVJ422P, AV_PIX_FMT_YUVJ444P, AV_PIX_FMT_NONE };
                ...
                if (strict_val > FF_COMPLIANCE_UNOFFICIAL) ofp->formats = mjpeg_formats;
            }
        }
        ...
        if (!ofp->fps.last_frame) return AVERROR(ENOMEM);

        ofp->fps.framerate = ost->frame_rate;
        ofp->fps.framerate_max = ost->max_frame_rate;
        ofp->fps.framerate_supported = ost->force_fps ? NULL : c->supported_framerates;

        // reduce frame rate for mpeg4 to be within the spec limits
        if (c->id == AV_CODEC_ID_MPEG4) ofp->fps.framerate_clip = 65535;
...
int fg_create(FilterGraph **pfg, char *graph_desc)
{
    ...
    if (!fgp->frame || !fgp->frame_enc) return AVERROR(ENOMEM);
    ...
    for (AVFilterInOut *cur = inputs; cur; cur = cur->next)
    {
        ...
        ifp->type = avfilter_pad_get_type(cur->filter_ctx->input_pads, cur->pad_idx);
    ...
    for (AVFilterInOut *cur = outputs; cur; cur = cur->next)
    {
        ...
        if (!ofilter) goto fail;

        ofilter->linklabel = cur->name;
        cur->name = NULL;

        ofilter->type = avfilter_pad_get_type(cur->filter_ctx->output_pads, cur->pad_idx);
...
int init_simple_filtergraph(InputStream *ist, OutputStream *ost, char *graph_desc)
{
    ...
    snprintf(fgp->log_name, sizeof(fgp->log_name), "%cf#%d:%d", av_get_media_type_string(ost->type)[0], ost->file_index, ost->index);
...
static int init_input_filter(FilterGraph *fg, InputFilter *ifilter)
{
    ...
    if (ifp->linklabel)
    {
        ...
        for (i = 0; i < s->nb_streams; i++)
        {
            ...
            if (stream_type != type && !(stream_type == AVMEDIA_TYPE_SUBTITLE && type == AVMEDIA_TYPE_VIDEO /* sub2video hack */)) continue;
    ...
    if (ret < 0)
    {
        av_log(fg, AV_LOG_ERROR, "Error binding an input stream to complex filtergraph input %s.\n", ifilter->name);
        return ret;
    }
...
static int insert_filter(AVFilterContext **last_filter, int *pad_idx, const char *filter_name, const char *args)
{
    ...
    ret = avfilter_graph_create_filter(&ctx, avfilter_get_by_name(filter_name), filter_name, args, NULL, graph);
    if (ret < 0) return ret;
...
static int configure_output_video_filter(FilterGraph *fg, OutputFilter *ofilter, AVFilterInOut *out)
{
    ...
    ret = avfilter_graph_create_filter(&ofp->filter, avfilter_get_by_name("buffersink"), name, NULL, NULL, fg->graph);
    ...
    if ((ofp->width || ofp->height) && ofilter->ost->autoscale)
    {
        ...
        if ((ret = avfilter_graph_create_filter(&filter, avfilter_get_by_name("scale"), name, args, NULL, fg->graph)) < 0) return ret;
    ...
    if (pix_fmts)
    {
        ...
        ret = avfilter_graph_create_filter(&filter, avfilter_get_by_name("format"), "format", pix_fmts, NULL, fg->graph);
    ...
    ret = insert_trim(of->start_time, of->recording_time, &last_filter, &pad_idx, name);
    if (ret < 0) return ret;

    if ((ret = avfilter_link(last_filter, pad_idx, ofp->filter, 0)) < 0) return ret;
...
static int configure_output_audio_filter(FilterGraph *fg, OutputFilter *ofilter, AVFilterInOut *out)
{
    ...
    snprintf(name, sizeof(name), "out_%d_%d", ost->file_index, ost->index);
    ret = avfilter_graph_create_filter(&ofp->filter, avfilter_get_by_name("abuffersink"), name, NULL, NULL, fg->graph);
    if (ret < 0) return ret;
    if ((ret = av_opt_set_int(ofp->filter, "all_channel_counts", 1, AV_OPT_SEARCH_CHILDREN)) < 0) return ret;
...
#define AUTO_INSERT_FILTER(opt_name, filter_name, arg)                                                                                                         \
    ...
    {                                                                                                                                                          \
        ...
        av_log(fg, AV_LOG_INFO,                                                                                                                                \
            ...
            arg);                                                                                                                                              \
                                                                                                                                                               \
        ret = avfilter_graph_create_filter(&filt_ctx, avfilter_get_by_name(filter_name), filter_name, arg, NULL, fg->graph);                                   \
        if (ret < 0) goto fail;                                                                                                                                \
                                                                                                                                                               \
    ...
    if (ost->audio_channels_mapped)
    {
        ...
        for (i = 0; i < ost->audio_channels_mapped; i++)
            if (ost->audio_channels_map[i] != -1) av_bprintf(&args, "|c%d=c%d", i, ost->audio_channels_map[i]);
    ...
    if (args.len)
    {
        ...
        snprintf(name, sizeof(name), "format_out_%d_%d", ost->file_index, ost->index);
        ret = avfilter_graph_create_filter(&format, avfilter_get_by_name("aformat"), name, args.str, NULL, fg->graph);
        if (ret < 0) goto fail;
    ...
    if (ost->apad && of->shortest)
    {
        ...
        for (i = 0; i < of->nb_streams; i++)
            if (of->streams[i]->st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) break;
    ...
    snprintf(name, sizeof(name), "trim for output stream %d:%d", ost->file_index, ost->index);
    ret = insert_trim(of->start_time, of->recording_time, &last_filter, &pad_idx, name);
    if (ret < 0) goto fail;

    if ((ret = avfilter_link(last_filter, pad_idx, ofp->filter, 0)) < 0) goto fail;
fail:
...
int check_filter_outputs(void)
{
    ...
    for (i = 0; i < nb_filtergraphs; i++)
    {
        ...
        for (n = 0; n < filtergraphs[i]->nb_outputs; n++)
        {
            ...
            if (!output->ost)
            {
                av_log(filtergraphs[i], AV_LOG_FATAL, "Filter %s has an unconnected output\n", output->name);
                return AVERROR(EINVAL);
            }
        }
    }
...
static int configure_input_video_filter(FilterGraph *fg, InputFilter *ifilter, AVFilterInOut *in)
{
    ...
    if (!fr.num) fr = ist->framerate_guessed;

    if (ifp->type_src == AVMEDIA_TYPE_SUBTITLE) sub2video_prepare(ifp);

    ifp->time_base = ist->framerate.num ? av_inv_q(ist->framerate) : ist->st->time_base;

    sar = ifp->sample_aspect_ratio;
    if (!sar.den) sar = (AVRational){ 0, 1 };
    av_bprint_init(&args, 0, AV_BPRINT_SIZE_AUTOMATIC);
    av_bprintf(&args,
        "video_size=%dx%d:pix_fmt=%d:time_base=%d/%d:"
        "pixel_aspect=%d/%d",
        ifp->width, ifp->height, ifp->format, ifp->time_base.num, ifp->time_base.den, sar.num, sar.den);
    if (fr.num && fr.den) av_bprintf(&args, ":frame_rate=%d/%d", fr.num, fr.den);
    snprintf(name, sizeof(name), "graph %d input from stream %d:%d", fg->index, ist->file_index, ist->index);

    if ((ret = avfilter_graph_create_filter(&ifp->filter, buffer_filt, name, args.str, NULL, fg->graph)) < 0) goto fail;
    ...
    if (ist->autorotate && !(desc->flags & AV_PIX_FMT_FLAG_HWACCEL))
    {
        ...
        if (!ifp->displaymatrix_present)
            sd = av_packet_side_data_get(ist->st->codecpar->coded_side_data, ist->st->codecpar->nb_coded_side_data, AV_PKT_DATA_DISPLAYMATRIX);
        if (sd) displaymatrix = (int32_t *)sd->data;
        theta = get_rotation(displaymatrix);

        if (fabs(theta - 90) < 1.0)
        {
            ret = insert_filter(&last_filter, &pad_idx, "transpose", displaymatrix[3] > 0 ? "cclock_flip" : "clock");
        }
        ...
        {
            ret = insert_filter(&last_filter, &pad_idx, "transpose", displaymatrix[3] < 0 ? "clock_flip" : "cclock");
        }
    ...
    if (copy_ts)
    {
        ...
        if (!start_at_zero && f->ctx->start_time != AV_NOPTS_VALUE) tsoffset += f->ctx->start_time;
    }
    ret = insert_trim(((f->start_time == AV_NOPTS_VALUE) || !f->accurate_seek) ? AV_NOPTS_VALUE : tsoffset, f->recording_time, &last_filter, &pad_idx, name);
    if (ret < 0) return ret;

    if ((ret = avfilter_link(last_filter, 0, in->filter_ctx, in->pad_idx)) < 0) return ret;
    return 0;
fail:
    av_freep(&par);
...
static int configure_input_audio_filter(FilterGraph *fg, InputFilter *ifilter, AVFilterInOut *in)
{
    ...
    av_bprint_init(&args, 0, AV_BPRINT_SIZE_AUTOMATIC);
    av_bprintf(
        &args, "time_base=%d/%d:sample_rate=%d:sample_fmt=%s", ifp->time_base.num, ifp->time_base.den, ifp->sample_rate, av_get_sample_fmt_name(ifp->format));
    if (av_channel_layout_check(&ifp->ch_layout) && ifp->ch_layout.order != AV_CHANNEL_ORDER_UNSPEC)
    {
    ...
    snprintf(name, sizeof(name), "graph_%d_in_%d_%d", fg->index, ist->file_index, ist->index);

    if ((ret = avfilter_graph_create_filter(&ifp->filter, abuffer_filt, name, args.str, NULL, fg->graph)) < 0) return ret;
    last_filter = ifp->filter;

    snprintf(name, sizeof(name), "trim for input stream %d:%d", ist->file_index, ist->index);
    if (copy_ts)
    {
        tsoffset = f->start_time == AV_NOPTS_VALUE ? 0 : f->start_time;
        if (!start_at_zero && f->ctx->start_time != AV_NOPTS_VALUE) tsoffset += f->ctx->start_time;
...
{
    return f->nb_inputs == 0 && (!strcmp(f->filter->name, "buffer") || !strcmp(f->filter->name, "abuffer"));
}
...
static int graph_is_meta(AVFilterGraph *graph)
{
    for (unsigned i = 0; i < graph->nb_filters; i++)
    {
         ...
         */
        if (!((f->filter->flags & AVFILTER_FLAG_METADATA_ONLY) || f->nb_outputs == 0 || filter_is_buffersrc(f))) return 0;
    }
    return 1;
}
...
static int configure_filtergraph(FilterGraph *fg)
{
    ...
    if ((ret = graph_parse(fg->graph, graph_desc, &inputs, &outputs, hw_device)) < 0) goto fail;
    ...
    if (fgp->disable_conversions) avfilter_graph_set_auto_convert(fg->graph, AVFILTER_AUTO_CONVERT_NONE);
    if ((ret = avfilter_graph_config(fg->graph, NULL)) < 0) goto fail;
    ...
    for (i = 0; i < fg->nb_outputs; i++)
    {
        ...
        if (!ofp->tb_out_locked)
        {
            ...
            if (ofp->fps.framerate.num <= 0 && ofp->fps.framerate.den <= 0 && fr.num > 0 && fr.den > 0) ofp->fps.framerate = fr;
...
static int ifilter_parameters_from_frame(InputFilter *ifilter, const AVFrame *frame)
{
    ...
    if (sd) memcpy(ifp->displaymatrix, sd->data, sizeof(ifp->displaymatrix));
...
void fg_send_command(FilterGraph *fg, double time, const char *target, const char *command, const char *arg, int all_filters)
{
    ...
    {
        char response[4096];
        ret = avfilter_graph_send_command(fg->graph, target, command, arg, response, sizeof(response), all_filters ? 0 : AVFILTER_CMD_FLAG_ONE);
        fprintf(stderr, "Command reply for stream %d: ret:%d res:\n%s", fg->index, ret, response);
    }
    else if (!all_filters)
    {
    ...
    else
    {
        ...
        if (ret < 0) fprintf(stderr, "Queuing command failed with error %s\n", av_err2str(ret));
    }
}
...
static int choose_out_timebase(OutputFilterPriv *ofp, AVFrame *frame)
{
    ...
    // apply -enc_time_base
    if (ofp->enc_timebase.num == ENC_TIME_BASE_DEMUX && (fd->dec.tb.num <= 0 || fd->dec.tb.den <= 0))
    {
        av_log(ofilter->ost, AV_LOG_ERROR, "Demuxing timebase not available - cannot use it for encoding\n");
        return AVERROR(EINVAL);
    }
    ...
    if (!fr.num)
    {
        ...
        if (fr_sink.num > 0 && fr_sink.den > 0) fr = fr_sink;
    }
    ...
    if (ofilter->ost->is_cfr)
    {
        ...
        if (fps->framerate_max.num && (av_q2d(fr) > av_q2d(fps->framerate_max) || !fr.den)) fr = fps->framerate_max;
    }
    ...
    if (!(tb.num > 0 && tb.den > 0)) tb = av_inv_q(fr);
    if (!(tb.num > 0 && tb.den > 0)) tb = frame->time_base;
...
static double adjust_frame_pts_to_encoder_tb(AVFrame *frame, AVRational tb_dst, int64_t start_time)
{
    ...
    if (frame->pts == AV_NOPTS_VALUE) goto early_exit;
    ...
    if (float_pts != llrint(float_pts)) float_pts += FFSIGN(float_pts) * 1.0 / (1 << 17);

    frame->pts = av_rescale_q(frame->pts, filter_tb, tb_dst) - av_rescale_q(start_time, AV_TIME_BASE_Q, tb_dst);
...
early_exit:
    ...
    if (debug_ts)
    {
        av_log(NULL, AV_LOG_INFO, "filter -> pts:%s pts_time:%s exact:%f time_base:%d/%d\n", frame ? av_ts2str(frame->pts) : "NULL",
            av_ts2timestr(frame->pts, &tb_dst), float_pts, tb_dst.num, tb_dst.den);
    }
...
static void video_sync_process(OutputFilterPriv *ofp, AVFrame *frame, int64_t *nb_frames, int64_t *nb_frames_prev)
{
    ...
    if (!frame)
    {
        *nb_frames_prev = *nb_frames = mid_pred(fps->frames_prev_hist[0], fps->frames_prev_hist[1], fps->frames_prev_hist[2]);
    ...
    *nb_frames = 1;
    ...
    if (delta0 < 0 && delta > 0 && ost->vsync_method != VSYNC_PASSTHROUGH && ost->vsync_method != VSYNC_DROP)
    {
        if (delta0 < -0.6)
        {
    ...
    case VSYNC_CFR:
        ...
        else if (delta > 1.1)
        {
            *nb_frames = llrintf(delta);
            if (delta0 > 1.1) *nb_frames_prev = llrintf(delta0 - 0.6);
        }
        frame->duration = 1;
        break;
    case VSYNC_VFR:
...
finish:
    memmove(fps->frames_prev_hist + 1, fps->frames_prev_hist, sizeof(fps->frames_prev_hist[0]) * (FF_ARRAY_ELEMS(fps->frames_prev_hist) - 1));
...
static int fg_output_frame(OutputFilterPriv *ofp, AVFrame *frame)
{
    ...
    if (type == AVMEDIA_TYPE_VIDEO) video_sync_process(ofp, frame, &nb_frames, &nb_frames_prev);
    ...
    for (int64_t i = 0; i < nb_frames; i++)
    {
        ...
        {
            AVFrame *frame_in = (i < nb_frames_prev && frame_prev->buf[0]) ? frame_prev : frame;
            if (!frame_in) break;
        ...
        {
            frame->pts = (frame->pts == AV_NOPTS_VALUE)
                             ? ofp->next_pts
                             : av_rescale_q(frame->pts, frame->time_base, ofp->tb_out) - av_rescale_q(ofp->ts_offset, AV_TIME_BASE_Q, ofp->tb_out);

            frame->time_base = ofp->tb_out;
            frame->duration = av_rescale_q(frame->nb_samples, (AVRational){ 1, frame->sample_rate }, ofp->tb_out);
        ...
        if (type == AVMEDIA_TYPE_VIDEO)
        {
            ...
            if (i == nb_frames_prev && frame) frame->flags &= ~AV_FRAME_FLAG_KEY;
        }
...
static int fg_output_step(OutputFilterPriv *ofp, int flush)
{
    ...
    ret = av_buffersink_get_frame_flags(filter, frame, AV_BUFFERSINK_FLAG_NO_REQUEST);
    if (flush && ret == AVERROR_EOF && ofp->got_frame && ost->type == AVMEDIA_TYPE_VIDEO)
    {
    ...
    {
        av_log(fgp, AV_LOG_WARNING, "Error in retrieving a frame from the filtergraph: %s\n", av_err2str(ret));
        return ret;
    }
    if (ost->finished)
    {
    ...
    {
        ost->filter->last_pts = av_rescale_q(frame->pts, frame->time_base, AV_TIME_BASE_Q);
    ...
    if (!fgp->is_meta) fd->bits_per_raw_sample = 0;
    ...
    if (ost->type == AVMEDIA_TYPE_VIDEO)
    {
        if (!frame->duration)
        {
            ...
            if (fr.num > 0 && fr.den > 0) frame->duration = av_rescale_q(1, av_inv_q(fr), frame->time_base);
        }
...
void ifilter_sub2video_heartbeat(InputFilter *ifilter, int64_t pts, AVRational tb)
{
    ...
    if (pts2 <= ifp->sub2video.last_pts) return;
    ...
    if (av_buffersrc_get_nb_failed_requests(ifp->filter)) sub2video_push_ref(ifp, pts2);
}
...
int ifilter_sub2video(InputFilter *ifilter, const AVFrame *frame)
{
    ...
    if (ifilter->graph->graph)
    {
        if (!frame)
        {
            if (ifp->sub2video.end_pts < INT64_MAX) sub2video_update(ifp, INT64_MAX, NULL);
...
int ifilter_send_eof(InputFilter *ifilter, int64_t pts, AVRational tb)
{
    ...
    if (ifp->filter)
    {
        pts = av_rescale_q_rnd(pts, tb, ifp->time_base, AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX);
    ...
    else
    {
        if (ifp->format < 0)
        {
            ...
            ret = av_channel_layout_copy(&ifp->ch_layout, &ifp->fallback.ch_layout);
            if (ret < 0) return ret;
...
int ifilter_send_frame(InputFilter *ifilter, AVFrame *frame, int keep_reference)
{
    ...
    {
    case AVMEDIA_TYPE_AUDIO:
        need_reinit |= ifp->sample_rate != frame->sample_rate || av_channel_layout_compare(&ifp->ch_layout, &frame->ch_layout);
        break;
    case AVMEDIA_TYPE_VIDEO:
        need_reinit |= ifp->width != frame->width || ifp->height != frame->height;
        break;
    }

    if (!ifp->ist->reinit_filters && fg->graph) need_reinit = 0;

    if (!!ifp->hw_frames_ctx != !!frame->hw_frames_ctx || (ifp->hw_frames_ctx && ifp->hw_frames_ctx->data != frame->hw_frames_ctx->data)) need_reinit = 1;

    if (sd = av_frame_get_side_data(frame, AV_FRAME_DATA_DISPLAYMATRIX))
    {
        if (!ifp->displaymatrix_present || memcmp(sd->data, ifp->displaymatrix, sizeof(ifp->displaymatrix))) need_reinit = 1;
    }
    ...
    ret = av_buffersrc_add_frame_flags(ifp->filter, frame, AV_BUFFERSRC_FLAG_PUSH);
    if (ret < 0)
    {
        av_frame_unref(frame);
        if (ret != AVERROR_EOF) av_log(fg, AV_LOG_ERROR, "Error while filtering: %s\n", av_err2str(ret));
        return ret;
    }
...
int fg_transcode_step(FilterGraph *graph, InputStream **best_ist)
{
    ...
    *best_ist = NULL;
    ...
    if (ret >= 0) return reap_filters(graph, 0);
    ...
    for (i = 0; i < graph->nb_inputs; i++)
    {
        ...
        ist = ifp->ist;
        if (input_files[ist->file_index]->eagain || ifp->eof) continue;
