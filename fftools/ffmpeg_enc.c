...
static int hw_device_setup_for_encode(OutputStream *ost, AVBufferRef *frames_ref)
{
    ...
    for (i = 0;; i++)
    {
        ...
        if (frames_ref && config->methods & AV_CODEC_HW_CONFIG_METHOD_HW_FRAMES_CTX &&
            (config->pix_fmt == AV_PIX_FMT_NONE || config->pix_fmt == ost->enc_ctx->pix_fmt))
        {
            av_log(ost->enc_ctx, AV_LOG_VERBOSE,
                "Using input "
                "frames context (format %s) with %s encoder.\n",
                av_get_pix_fmt_name(ost->enc_ctx->pix_fmt), ost->enc_ctx->codec->name);
            ost->enc_ctx->hw_frames_ctx = av_buffer_ref(frames_ref);
            if (!ost->enc_ctx->hw_frames_ctx) return AVERROR(ENOMEM);
            return 0;
        }

        if (!dev && config->methods & AV_CODEC_HW_CONFIG_METHOD_HW_DEVICE_CTX) dev = hw_device_get_by_type(config->device_type);
    }
    ...
    if (dev)
    {
        av_log(ost->enc_ctx, AV_LOG_VERBOSE,
            ...
            dev->name, av_hwdevice_get_type_name(dev->type), ost->enc_ctx->codec->name);
        ost->enc_ctx->hw_device_ctx = av_buffer_ref(dev->device_ref);
        if (!ost->enc_ctx->hw_device_ctx) return AVERROR(ENOMEM);
    }
    else
    {
...
static int set_encoder_id(OutputFile *of, OutputStream *ost)
{
    ...
    if (av_dict_get(ost->st->metadata, "encoder", NULL, 0)) return 0;

    encoder_string_len = sizeof(LIBAVCODEC_IDENT) + strlen(cname) + 2;
    encoder_string = av_mallocz(encoder_string_len);
    if (!encoder_string) return AVERROR(ENOMEM);
...
int enc_open(OutputStream *ost, const AVFrame *frame)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        if (ost->bits_per_raw_sample)
            ...
            enc_ctx->bits_per_raw_sample = FFMIN(fd->bits_per_raw_sample, av_get_bytes_per_sample(enc_ctx->sample_fmt) << 3);
        break;
    ...
    case AVMEDIA_TYPE_VIDEO: {
        ...
        if (ost->bits_per_raw_sample)
            ...
            enc_ctx->bits_per_raw_sample = FFMIN(fd->bits_per_raw_sample, av_pix_fmt_desc_get(enc_ctx->pix_fmt)->comp[0].depth);
        ...
        if (enc_ctx->flags & (AV_CODEC_FLAG_INTERLACED_DCT | AV_CODEC_FLAG_INTERLACED_ME) || (frame->flags & AV_FRAME_FLAG_INTERLACED)
#if FFMPEG_OPT_TOP
        ...
        )
        {
            int top_field_first =
#if FFMPEG_OPT_TOP
                ost->top_field_first >= 0 ? ost->top_field_first :
#endif
    ...
    case AVMEDIA_TYPE_SUBTITLE:
        if (ost->enc_timebase.num) av_log(ost, AV_LOG_WARNING, "-enc_time_base not supported for subtitles, ignoring\n");
        ...
        if (dec_ctx && dec_ctx->subtitle_header)
        {
            ...
            if (!enc_ctx->subtitle_header) return AVERROR(ENOMEM);
            memcpy(enc_ctx->subtitle_header, dec_ctx->subtitle_header, dec_ctx->subtitle_header_size);
    ...
    if (ost->bitexact) enc_ctx->flags |= AV_CODEC_FLAG_BITEXACT;

    if (!av_dict_get(ost->encoder_opts, "threads", NULL, 0)) av_dict_set(&ost->encoder_opts, "threads", "auto", 0);
    ...
    if (ost->sq_idx_encode >= 0)
    {
        ...
        if (!e->sq_frame) return AVERROR(ENOMEM);
    }

    if (ost->enc_ctx->frame_size)
    {
        av_assert0(ost->sq_idx_encode >= 0);
        sq_frame_samples(output_files[ost->file_index]->sq_encode, ost->sq_idx_encode, ost->enc_ctx->frame_size);
    }
    ...
    if (ret < 0)
    {
        av_log(ost, AV_LOG_FATAL, "Error initializing the output stream codec context.\n");
        return ret;
    }
        ...
        for (i = 0; i < ist->st->codecpar->nb_coded_side_data; i++)
        {
            ...
            if (sd_src->type != AV_PKT_DATA_CPB_PROPERTIES)
            {
                AVPacketSideData *sd_dst =
                    av_packet_side_data_new(&ost->par_in->coded_side_data, &ost->par_in->nb_coded_side_data, sd_src->type, sd_src->size, 0);
                if (!sd_dst) return AVERROR(ENOMEM);
                memcpy(sd_dst->data, sd_src->data, sd_src->size);
                if (ist->autorotate && sd_src->type == AV_PKT_DATA_DISPLAYMATRIX) av_display_rotation_set((int32_t *)sd_dst->data, 0);
            }
        }
    }

    // copy timebase while removing common factors
    if (ost->st->time_base.num <= 0 || ost->st->time_base.den <= 0) ost->st->time_base = av_add_q(ost->enc_ctx->time_base, (AVRational){ 0, 1 });
...
static int check_recording_time(OutputStream *ost, int64_t ts, AVRational tb)
{
    ...
    if (of->recording_time != INT64_MAX && av_compare_ts(ts, tb, of->recording_time, AV_TIME_BASE_Q) >= 0)
    {
...
int enc_subtitle(OutputFile *of, OutputStream *ost, const AVSubtitle *sub)
{
    ...
    if (ost->finished || (of->start_time != AV_NOPTS_VALUE && sub->pts < of->start_time)) return 0;
    ...
    pts = sub->pts;
    if (output_files[ost->file_index]->start_time != AV_NOPTS_VALUE) pts -= output_files[ost->file_index]->start_time;
    for (i = 0; i < nb; i++)
    {
        AVSubtitle local_sub = *sub;

        if (!check_recording_time(ost, pts, AV_TIME_BASE_Q)) return 0;

        ret = av_new_packet(pkt, subtitle_out_max_size);
        if (ret < 0) return AVERROR(ENOMEM);
...
static int update_video_stats(OutputStream *ost, const AVPacket *pkt, int write_vstats)
{
    ...
    if ((enc->flags & AV_CODEC_FLAG_PSNR) && sd && sd[5])
    {
        ...
        if (error >= 0 && error <= 1) psnr_val = psnr(error);
    }

    if (!write_vstats) return 0;
    ...
    if (vstats_version <= 1)
    {
        fprintf(vstats_file, "frame= %5" PRId64 " q= %2.1f ", frame_number, ost->quality / (float)FF_QP2LAMBDA);
    }
    else
    {
        fprintf(vstats_file, "out= %2d st= %2d frame= %5" PRId64 " q= %2.1f ", ost->file_index, ost->index, frame_number, ost->quality / (float)FF_QP2LAMBDA);
    }

    if (psnr_val >= 0) fprintf(vstats_file, "PSNR= %6.2f ", psnr_val);
    ...
    fprintf(vstats_file, "s_size= %8.0fkB time= %0.3f br= %7.1fkbits/s avg_br= %7.1fkbits/s ", (double)e->data_size / 1024, ti1, bitrate, avg_bitrate);
...
static int encode_frame(OutputFile *of, OutputStream *ost, AVFrame *frame)
{
    ...
    if (frame)
    {
        if (ost->enc_stats_pre.io) enc_stats_write(ost, &ost->enc_stats_pre, frame, NULL, ost->frames_encoded);
        ...
        if (debug_ts)
        {
            av_log(ost, AV_LOG_INFO,
                ...
                type_desc, av_ts2str(frame->pts), av_ts2timestr(frame->pts, &enc->time_base), enc->time_base.num, enc->time_base.den);
        }

        if (frame->sample_aspect_ratio.num && !ost->frame_aspect_ratio.num) enc->sample_aspect_ratio = frame->sample_aspect_ratio;
    }
    ...
    while (1)
    {
        ...
        update_benchmark("%s_%s %d.%d", action, type_desc, ost->file_index, ost->index);

        pkt->time_base = enc->time_base;

        /* if two pass, output log on success and EOF */
        if ((ret >= 0 || ret == AVERROR_EOF) && ost->logfile && enc->stats_out) fprintf(ost->logfile, "%s", enc->stats_out);
        ...
        if (ost->enc_stats_post.io) enc_stats_write(ost, &ost->enc_stats_post, NULL, pkt, e->packets_encoded);
        ...
        if (debug_ts)
        {
            av_log(ost, AV_LOG_INFO,
                ...
                type_desc, av_ts2str(pkt->pts), av_ts2timestr(pkt->pts, &enc->time_base), av_ts2str(pkt->dts), av_ts2timestr(pkt->dts, &enc->time_base),
                av_ts2str(pkt->duration), av_ts2timestr(pkt->duration, &enc->time_base));
        }

        if ((ret = trigger_fix_sub_duration_heartbeat(ost, pkt)) < 0)
        {
            av_log(NULL, AV_LOG_ERROR, "Subtitle heartbeat logic failed in %s! (%s)\n", __func__, av_err2str(ret));
            return ret;
        }
...
static int submit_encode_frame(OutputFile *of, OutputStream *ost, AVFrame *frame)
{
    ...
    while (1)
    {
        ...
        if (enc_frame) av_frame_unref(enc_frame);
        if (ret < 0)
        {
            if (ret == AVERROR_EOF) close_output_stream(ost);
            return ret;
        }
    }
}
...
static int do_audio_out(OutputFile *of, OutputStream *ost, AVFrame *frame)
{
    ...
    if (!(enc->codec->capabilities & AV_CODEC_CAP_PARAM_CHANGE) && enc->ch_layout.nb_channels != frame->ch_layout.nb_channels)
    {
        av_log(ost, AV_LOG_ERROR, "Audio channel count changed and encoder does not support parameter changes\n");
        return 0;
    }

    if (!check_recording_time(ost, frame->pts, frame->time_base)) return 0;
...
static enum AVPictureType forced_kf_apply(void *logctx, KeyframeForceCtx *kf, AVRational tb, const AVFrame *in_picture)
{
    ...
    if (kf->ref_pts == AV_NOPTS_VALUE) kf->ref_pts = in_picture->pts;

    pts_time = (in_picture->pts - kf->ref_pts) * av_q2d(tb);
    if (kf->index < kf->nb_pts && av_compare_ts(in_picture->pts, tb, kf->pts[kf->index], AV_TIME_BASE_Q) >= 0)
    {
        kf->index++;
        goto force_keyframe;
    }
    else if (kf->pexpr)
    {
        ...
        av_log(logctx, AV_LOG_TRACE, "force_key_frame: n:%f n_forced:%f prev_forced_n:%f t:%f prev_forced_t:%f -> res:%f\n", kf->expr_const_values[FKF_N],
            kf->expr_const_values[FKF_N_FORCED], kf->expr_const_values[FKF_PREV_FORCED_N], kf->expr_const_values[FKF_T],
...
int enc_frame(OutputStream *ost, AVFrame *frame)
{
    ...
    return ost->enc_ctx->codec_type == AVMEDIA_TYPE_VIDEO ? do_video_out(of, ost, frame) : do_audio_out(of, ost, frame);
}
...
int enc_flush(void)
{
    ...
    for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost))
    {
        ...
        if (!enc || !e->opened || (enc->codec_type != AVMEDIA_TYPE_VIDEO && enc->codec_type != AVMEDIA_TYPE_AUDIO)) continue;
