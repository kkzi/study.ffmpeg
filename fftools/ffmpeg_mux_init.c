...
static char *get_line(AVIOContext *s, AVBPrint *bprint)
{
    ...
    if (!av_bprint_is_complete(bprint)) return NULL;
...
static int enc_stats_get_file(AVIOContext **io, const char *path)
{
    ...
    if (!esf->path) return AVERROR(ENOMEM);
    ...
    while (*next)
    {
        ...
        for (size_t i = 0; i < FF_ARRAY_ELEMS(fmt_specs); i++)
        {
            if (!strcmp(val, fmt_specs[i].str))
            {
                ...
                {
                    av_log(NULL, AV_LOG_ERROR, "Format directive '%s' may only be used %s-encoding\n", val, pre ? "post" : "pre");
                    ret = AVERROR(EINVAL);
                    goto fail;
                }
...
static MuxStream *mux_stream_alloc(Muxer *mux, enum AVMediaType type)
{
    ...
    snprintf(ms->log_name, sizeof(ms->log_name), "%cost#%d:%d", type_str ? *type_str : '?', mux->of.index, ms->ost.index);
    ...
    if (!ost->enc)
    {
        if (filters_script || filters)
        {
            av_log(ost, AV_LOG_ERROR,
                ...
                filters ? "Filtergraph" : "Filtergraph script", filters ? filters : filters_script);
            return AVERROR(ENOSYS);
        }
        return 0;
    }
...
static int parse_matrix_coeffs(void *logctx, uint16_t *dest, const char *str)
{
    ...
    for (i = 0;; i++)
    {
        ...
        if (!p)
        {
            av_log(logctx, AV_LOG_FATAL, "Syntax error in matrix \"%s\" at coeff %d\n", str, i);
            return AVERROR(EINVAL);
        }
        p++;
    }
    ...
    if (*p == AV_PIX_FMT_NONE)
    {
        if (target != AV_PIX_FMT_NONE)
            av_log(NULL, AV_LOG_WARNING, "Incompatible pixel format '%s' for codec '%s', auto-selecting format '%s'\n", av_get_pix_fmt_name(target),
                codec->name, av_get_pix_fmt_name(best));
        return best;
    }
    return target;
}
...
static enum AVPixelFormat pix_fmt_parse(OutputStream *ost, const char *name)
{
    ...
    if (fmts && !fmt_in_list(fmts, fmt))
    {
        ...
        if (strcmp(name, name_canonical) && (!strcmp(name_canonical + len - 2, "le") || !strcmp(name_canonical + len - 2, "be")))
        {
            char name_other[64];
            enum AVPixelFormat fmt_other;

            snprintf(name_other, sizeof(name_other), "%s%ce", name, name_canonical[len - 2] == 'l' ? 'b' : 'l');
    ...
    if (fmts && !fmt_in_list(fmts, fmt)) fmt = choose_pixel_fmt(ost->enc_ctx->codec, fmt);
...
static int new_stream_video(Muxer *mux, const OptionsContext *o, OutputStream *ost)
{
    ...
    if (ost->enc_ctx)
    {
        ...
        {
            ost->keep_pix_fmt = 1;
            if (!*++frame_pix_fmt) frame_pix_fmt = NULL;
        }
        if (frame_pix_fmt)
        {
            video_enc->pix_fmt = pix_fmt_parse(ost, frame_pix_fmt);
            if (video_enc->pix_fmt == AV_PIX_FMT_NONE) return AVERROR(EINVAL);
        }

        MATCH_PER_STREAM_OPT(intra_matrices, str, intra_matrix, oc, st);
        if (intra_matrix)
        {
            if (!(video_enc->intra_matrix = av_mallocz(sizeof(*video_enc->intra_matrix) * 64))) return AVERROR(ENOMEM);
        ...
        if (inter_matrix)
        {
            if (!(video_enc->inter_matrix = av_mallocz(sizeof(*video_enc->inter_matrix) * 64))) return AVERROR(ENOMEM);
        ...
        if (ost->logfile_prefix && !(ost->logfile_prefix = av_strdup(ost->logfile_prefix))) return AVERROR(ENOMEM);
        ...
        if (do_pass)
        {
            ...
            snprintf(logfilename, sizeof(logfilename), "%s-%d.log", ost->logfile_prefix ? ost->logfile_prefix : DEFAULT_PASS_LOGFILENAME_PREFIX, ost_idx);
            ...
            else
            {
                ...
                if (video_enc->flags & AV_CODEC_FLAG_PASS1)
                {
                    ...
                    if (!f)
                    {
                        av_log(ost, AV_LOG_FATAL, "Cannot write log file '%s' for pass-1 encoding: %s\n", logfilename, strerror(errno));
        ...
        if (ost->top_field_first >= 0) av_log(ost, AV_LOG_WARNING, "-top is deprecated, use the setfield filter instead\n");
#endif
        ...
        if ((ost->frame_rate.num || ost->max_frame_rate.num) &&
            !(ost->vsync_method == VSYNC_AUTO || ost->vsync_method == VSYNC_CFR || ost->vsync_method == VSYNC_VSCFR))
        {
            av_log(ost, AV_LOG_FATAL,
        ...
        if (ost->vsync_method == VSYNC_AUTO)
        {
            ...
            {
                ost->vsync_method =
                    (oc->oformat->flags & AVFMT_VARIABLE_FPS) ? ((oc->oformat->flags & AVFMT_NOTIMESTAMPS) ? VSYNC_PASSTHROUGH : VSYNC_VFR) : VSYNC_CFR;
            }
            ...
            if (ost->ist && ost->vsync_method == VSYNC_CFR)
            {
                ...
                if (ifile->nb_streams == 1 && ifile->input_ts_offset == 0) ost->vsync_method = VSYNC_VSCFR;
            }
    ...
    if (ost->enc_ctx)
    {
        ...
        for (int n = 0; n < o->nb_audio_channel_maps; n++)
        {
            ...
            if ((map->ofile_idx == -1 || ost->file_index == map->ofile_idx) && (map->ostream_idx == -1 || ost->st->index == map->ostream_idx))
            {
                ...
                {
                    ret = av_reallocp_array(&ost->audio_channels_map, ost->audio_channels_mapped + 1, sizeof(*ost->audio_channels_map));
                    if (ret < 0) return ret;
...
static int new_stream_subtitle(Muxer *mux, const OptionsContext *o, OutputStream *ost)
{
    ...
    if (ost->enc_ctx)
    {
        ...
        AVCodecDescriptor const *input_descriptor = avcodec_descriptor_get(ost->ist->par->codec_id);
        AVCodecDescriptor const *output_descriptor = avcodec_descriptor_get(subtitle_enc->codec_id);
        ...
        if (input_descriptor) input_props = input_descriptor->props & (AV_CODEC_PROP_TEXT_SUB | AV_CODEC_PROP_BITMAP_SUB);
        if (output_descriptor) output_props = output_descriptor->props & (AV_CODEC_PROP_TEXT_SUB | AV_CODEC_PROP_BITMAP_SUB);
...
static int streamcopy_init(const Muxer *mux, OutputStream *ost)
{
    ...
    if (ret >= 0) ret = av_opt_set_dict(codec_ctx, &ost->encoder_opts);
    if (ret < 0)
    {
    ...
    if (!codec_tag)
    {
        ...
        if (!ct || av_codec_get_id(ct, par->codec_tag) == par->codec_id || !av_codec_get_tag2(ct, par->codec_id, &codec_tag_tmp)) codec_tag = par->codec_tag;
    }

    par->codec_tag = codec_tag;

    if (!fr.num) fr = ist->framerate;
    ...
    ret = avformat_transfer_internal_stream_timing_info(mux->fc->oformat, ost->st, ist->st, copy_tb);
    if (ret < 0) goto fail;
    ...
    {
        ms->ts_copy_start = (mux->of.start_time == AV_NOPTS_VALUE) ? 0 : mux->of.start_time;
        if (copy_ts && ifile->start_time != AV_NOPTS_VALUE)
        {
            ms->ts_copy_start = FFMAX(ms->ts_copy_start, ifile->start_time + ifile->ts_offset);
        }
    }
    ...
    for (int i = 0; i < ist->st->codecpar->nb_coded_side_data; i++)
    {
        ...
        sd_dst = av_packet_side_data_new(&ost->st->codecpar->coded_side_data, &ost->st->codecpar->nb_coded_side_data, sd_src->type, sd_src->size, 0);
        if (!sd_dst)
        {
            ret = AVERROR(ENOMEM);
            goto fail;
    ...
    if (ost->rotate_overridden)
    {
        AVPacketSideData *sd = av_packet_side_data_new(
            &ost->st->codecpar->coded_side_data, &ost->st->codecpar->nb_coded_side_data, AV_PKT_DATA_DISPLAYMATRIX, sizeof(int32_t) * 9, 0);
        if (sd) av_display_rotation_set((int32_t *)sd->data, -ost->rotate_override_value);
    }
#endif
    ...
    case AVMEDIA_TYPE_AUDIO:
        if ((par->block_align == 1 || par->block_align == 1152 || par->block_align == 576) && par->codec_id == AV_CODEC_ID_MP3) par->block_align = 0;
        if (par->codec_id == AV_CODEC_ID_AC3) par->block_align = 0;
        break;
    case AVMEDIA_TYPE_VIDEO: {
        AVRational sar;
        if (ost->frame_aspect_ratio.num)
        {  // overridden by the -aspect cli option
            sar = av_mul_q(ost->frame_aspect_ratio, (AVRational){ par->height, par->width });
            av_log(ost, AV_LOG_WARNING,
...
static int ost_add(Muxer *mux, const OptionsContext *o, enum AVMediaType type, InputStream *ist, OutputFilter *ofilter, OutputStream **post)
{
    ...
    if (!ost->par_in) return AVERROR(ENOMEM);

    ms->muxing_queue = av_fifo_alloc2(8, sizeof(AVPacket *), 0);
    if (!ms->muxing_queue) return AVERROR(ENOMEM);
    ...
    if (enc)
    {
        ...
        if (!ost->enc_ctx) return AVERROR(ENOMEM);
    ...
    av_log(ost, AV_LOG_VERBOSE, "Created %s stream from ", av_get_media_type_string(type));
    if (ist)
        av_log(ost, AV_LOG_VERBOSE, "input stream %d:%d", ist->file_index, ist->index);
    else if (ofilter)
    ...
    if (!ms->pkt) return AVERROR(ENOMEM);
    ...
    if (ost->enc_ctx)
    {
        ...
        ret = filter_codec_opts(o->g->codec_opts, enc->codec_id, oc, st, enc->codec, &ost->encoder_opts);
        if (ret < 0) return ret;
        ...
        if (ret)
        {
            av_log(ost, AV_LOG_FATAL, "Preset %s specified, but could not be opened.\n", preset);
            return ret;
        }

        MATCH_PER_STREAM_OPT(enc_stats_pre, str, enc_stats_pre, oc, st);
        if (enc_stats_pre && (type == AVMEDIA_TYPE_VIDEO || type == AVMEDIA_TYPE_AUDIO))
        {
        ...
        if (enc_stats_post && (type == AVMEDIA_TYPE_VIDEO || type == AVMEDIA_TYPE_AUDIO))
        {
            const char *format = "{fidx} {sidx} {n} {t}";
        ...
        if (mux_stats && (type == AVMEDIA_TYPE_VIDEO || type == AVMEDIA_TYPE_AUDIO))
        {
            const char *format = "{fidx} {sidx} {n} {t}";
    ...
    {
        ret = filter_codec_opts(o->g->codec_opts, AV_CODEC_ID_NONE, oc, st, NULL, &ost->encoder_opts);
        if (ret < 0) return ret;
    }
    ...
    {
        ost->bitexact = check_opt_bitexact(ost->enc_ctx, ost->encoder_opts, "flags", AV_CODEC_FLAG_BITEXACT);
    }
    ...
    if (codec_tag)
    {
        ...
        if (ost->enc_ctx) ost->enc_ctx->codec_tag = tag;
    }
    ...
    MATCH_PER_STREAM_OPT(bits_per_raw_sample, i, ost->bits_per_raw_sample, oc, st);

    MATCH_PER_STREAM_OPT(fix_sub_duration_heartbeat, i, ost->fix_sub_duration_heartbeat, oc, st);

    if (oc->oformat->flags & AVFMT_GLOBALHEADER && ost->enc_ctx) ost->enc_ctx->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;

    av_dict_copy(&ost->sws_dict, o->g->sws_dict, 0);

    av_dict_copy(&ost->swr_opts, o->g->swr_opts, 0);
    if (ost->enc_ctx && av_get_exact_bits_per_sample(ost->enc_ctx->codec_id) == 24) av_dict_set(&ost->swr_opts, "output_sample_bits", "24", 0);

    ost->last_mux_dts = AV_NOPTS_VALUE;

    MATCH_PER_STREAM_OPT(copy_initial_nonkeyframes, i, ms->copy_initial_nonkeyframes, oc, st);
    ...
    if (ost->enc && (type == AVMEDIA_TYPE_VIDEO || type == AVMEDIA_TYPE_AUDIO))
    {
        ...
        else
        {
            ...
            if (ret < 0)
            {
                av_log(ost, AV_LOG_ERROR, "Error initializing a simple filtergraph\n");
                return ret;
            }
        }
    }
...
static int map_auto_video(Muxer *mux, const OptionsContext *o)
{
    ...
    if (av_guess_codec(oc->oformat, NULL, oc->url, NULL, AVMEDIA_TYPE_VIDEO) == AV_CODEC_ID_NONE) return 0;
    ...
    for (int j = 0; j < nb_input_files; j++)
    {
        ...
        for (int i = 0; i < ifile->nb_streams; i++)
        {
            ...
            if (ist->user_set_discard == AVDISCARD_ALL || ist->st->codecpar->codec_type != AVMEDIA_TYPE_VIDEO) continue;

            score = ist->st->codecpar->width * ist->st->codecpar->height + 100000000 * !!(ist->st->event_flags & AVSTREAM_EVENT_FLAG_NEW_PACKETS) +
                    5000000 * !!(ist->st->disposition & AV_DISPOSITION_DEFAULT);
            if ((qcr != MKTAG('A', 'P', 'I', 'C')) && (ist->st->disposition & AV_DISPOSITION_ATTACHED_PIC)) score = 1;
        ...
        if (file_best_ist)
        {
            if ((qcr == MKTAG('A', 'P', 'I', 'C')) || !(file_best_ist->st->disposition & AV_DISPOSITION_ATTACHED_PIC))
    ...
    if (best_ist) return ost_add(mux, o, AVMEDIA_TYPE_VIDEO, best_ist, NULL, NULL);
...
static int map_auto_audio(Muxer *mux, const OptionsContext *o)
{
    ...
    /* audio: most channels */
    if (av_guess_codec(oc->oformat, NULL, oc->url, NULL, AVMEDIA_TYPE_AUDIO) == AV_CODEC_ID_NONE) return 0;
    ...
    for (int j = 0; j < nb_input_files; j++)
    {
        ...
        for (int i = 0; i < ifile->nb_streams; i++)
        {
            ...
            if (ist->user_set_discard == AVDISCARD_ALL || ist->st->codecpar->codec_type != AVMEDIA_TYPE_AUDIO) continue;

            score = ist->st->codecpar->ch_layout.nb_channels + 100000000 * !!(ist->st->event_flags & AVSTREAM_EVENT_FLAG_NEW_PACKETS) +
                    5000000 * !!(ist->st->disposition & AV_DISPOSITION_DEFAULT);
            if (score > file_best_score)
            {
                file_best_score = score;
    ...
    if (best_ist) return ost_add(mux, o, AVMEDIA_TYPE_AUDIO, best_ist, NULL, NULL);
...
static int map_auto_subtitle(Muxer *mux, const OptionsContext *o)
{
    ...
    if (!avcodec_find_encoder(oc->oformat->subtitle_codec) && !subtitle_codec_name) return 0;

    for (InputStream *ist = ist_iter(NULL); ist; ist = ist_iter(ist))
        if (ist->st->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE)
        {
            AVCodecDescriptor const *input_descriptor = avcodec_descriptor_get(ist->st->codecpar->codec_id);
            ...
            if (output_codec) output_descriptor = avcodec_descriptor_get(output_codec->id);
            if (input_descriptor) input_props = input_descriptor->props & (AV_CODEC_PROP_TEXT_SUB | AV_CODEC_PROP_BITMAP_SUB);
            if (output_descriptor) output_props = output_descriptor->props & (AV_CODEC_PROP_TEXT_SUB | AV_CODEC_PROP_BITMAP_SUB);
            if (subtitle_codec_name || input_props & output_props ||
                // Map dvb teletext which has neither property to any output subtitle encoder
                input_descriptor && output_descriptor && (!input_descriptor->props || !output_descriptor->props))
            {
...
static int map_manual(Muxer *mux, const OptionsContext *o, const StreamMap *map)
{
    ...
    else
    {
        ...
        if (o->subtitle_disable && ist->st->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE) return 0;
        ...
        if (o->data_disable && ist->st->codecpar->codec_type == AVMEDIA_TYPE_DATA) return 0;

        if (ist->st->codecpar->codec_type == AVMEDIA_TYPE_UNKNOWN && !copy_unknown_streams)
        {
            av_log(mux, ignore_unknown_streams ? AV_LOG_WARNING : AV_LOG_FATAL, "Cannot map stream #%d:%d - unsupported type.\n", map->file_index,
                map->stream_index);
...
static int create_streams(Muxer *mux, const OptionsContext *o)
{
    ...
    int auto_disable = o->video_disable * (1 << AVMEDIA_TYPE_VIDEO) | o->audio_disable * (1 << AVMEDIA_TYPE_AUDIO) |
                       o->subtitle_disable * (1 << AVMEDIA_TYPE_SUBTITLE) | o->data_disable * (1 << AVMEDIA_TYPE_DATA);
    ...
    for (int i = 0; i < nb_filtergraphs; i++)
    {
        ...
        for (int j = 0; j < fg->nb_outputs; j++)
        {
            ...
            if (ofilter->linklabel || ofilter->ost) continue;
            ...
            av_log(mux, AV_LOG_VERBOSE,
                ...
                fg->index);
            if (!o->nb_stream_maps) av_log(mux, AV_LOG_VERBOSE, " This overrides automatic %s mapping.", av_get_media_type_string(ofilter->type));
            av_log(mux, AV_LOG_VERBOSE, "\n");
    ...
    if (!o->nb_stream_maps)
    {
        ...
        {
            if (!map_func[i] || auto_disable & (1 << i)) continue;
...
#define IS_INTERLEAVED(type) (type != AVMEDIA_TYPE_ATTACHMENT)
    ...
    for (int i = 0; i < oc->nb_streams; i++)
    {
        ...
        nb_audio_fs += (ost->enc_ctx && type == AVMEDIA_TYPE_AUDIO && !(ost->enc_ctx->codec->capabilities & AV_CODEC_CAP_VARIABLE_FRAME_SIZE));

        limit_frames |= ms->max_frames < INT64_MAX;
        limit_frames_av_enc |= (ms->max_frames < INT64_MAX) && IS_AV_ENC(ost, type);
    }

    if (!((nb_interleaved > 1 && of->shortest) || (nb_interleaved > 0 && limit_frames) || nb_audio_fs)) return 0;
    ...
    if ((of->shortest && nb_av_enc > 1) || limit_frames_av_enc || nb_audio_fs)
    {
        ...
        if (!of->sq_encode) return AVERROR(ENOMEM);
        ...
        for (int i = 0; i < oc->nb_streams; i++)
        {
            ...
            if (!IS_AV_ENC(ost, type)) continue;

            ost->sq_idx_encode = sq_add_stream(of->sq_encode, of->shortest || ms->max_frames < INT64_MAX);
            if (ost->sq_idx_encode < 0) return ost->sq_idx_encode;

            if (ms->max_frames != INT64_MAX) sq_limit_frames(of->sq_encode, ost->sq_idx_encode, ms->max_frames);
        }
    }
    ...
    if (nb_interleaved > nb_av_enc)
    {
        ...
        if (!mux->sq_mux) return AVERROR(ENOMEM);

        mux->sq_pkt = av_packet_alloc();
        if (!mux->sq_pkt) return AVERROR(ENOMEM);
        ...
        for (int i = 0; i < oc->nb_streams; i++)
        {
            ...
            if (!IS_INTERLEAVED(type)) continue;

            ost->sq_idx_mux = sq_add_stream(mux->sq_mux, of->shortest || ms->max_frames < INT64_MAX);
            if (ost->sq_idx_mux < 0) return ost->sq_idx_mux;

            if (ms->max_frames != INT64_MAX) sq_limit_frames(mux->sq_mux, ost->sq_idx_mux, ms->max_frames);
        }
    }
...
static int of_add_programs(Muxer *mux, const OptionsContext *o)
{
    ...
    for (int i = 0; i < o->nb_program; i++)
    {
        ...
        ret = av_dict_parse_string(&dict, o->program[i].u.str, "=", ":", AV_DICT_MULTIKEY);
        if (ret < 0)
        {
...
static int parse_meta_type(void *logctx, const char *arg, char *type, int *index, const char **stream_spec)
{
    if (*arg)
    {
        *type = *arg;
        ...
        case 'c':
        case 'p':
            if (*(++arg) == ':') *index = strtol(++arg, NULL, 0);
            break;
        default:
...
static int copy_chapters(InputFile *ifile, OutputFile *ofile, AVFormatContext *os, int copy_metadata)
{
    ...
    for (i = 0; i < is->nb_chapters; i++)
    {
        ...
        if (in_ch->end < ts_off) continue;
        if (rt != INT64_MAX && in_ch->start > rt + ts_off) break;

        out_ch = av_mallocz(sizeof(AVChapter));
        if (!out_ch) return AVERROR(ENOMEM);
        ...
        if (copy_metadata) av_dict_copy(&out_ch->metadata, in_ch->metadata, 0);
...
static int copy_metadata(Muxer *mux, AVFormatContext *ic, const char *outspec, const char *inspec, int *metadata_global_manual, int *metadata_streams_manual,
    int *metadata_chapters_manual)
{
    ...
    if (ret >= 0) ret = parse_meta_type(mux, outspec, &type_out, &idx_out, &ostream_spec);
    if (ret < 0) return ret;

    if (type_in == 'g' || type_out == 'g' || (!*outspec && !ic)) *metadata_global_manual = 1;
    if (type_in == 's' || type_out == 's' || (!*outspec && !ic)) *metadata_streams_manual = 1;
    if (type_in == 'c' || type_out == 'c' || (!*outspec && !ic)) *metadata_chapters_manual = 1;
...
static int copy_meta(Muxer *mux, const OptionsContext *o)
{
    ...
    for (int i = 0; i < o->nb_metadata_map; i++)
    {
        ...
        ret = copy_metadata(mux, in_file_index >= 0 ? input_files[in_file_index]->ctx : NULL, o->metadata_map[i].specifier, *p ? p + 1 : p,
            &metadata_global_manual, &metadata_streams_manual, &metadata_chapters_manual);
        if (ret < 0) return ret;
    }
    ...
    if (chapters_input_file >= 0) copy_chapters(input_files[chapters_input_file], of, oc, !metadata_chapters_manual);

    /* copy global metadata by default */
    if (!metadata_global_manual && nb_input_files)
    {
        av_dict_copy(&oc->metadata, input_files[0]->ctx->metadata, AV_DICT_DONT_OVERWRITE);
        if (of->recording_time != INT64_MAX) av_dict_set(&oc->metadata, "duration", NULL, 0);
...
static int set_dispositions(Muxer *mux, const OptionsContext *o)
{
    ...
    if (!dispositions) return AVERROR(ENOMEM);
    ...
    for (int i = 0; i < ctx->nb_streams; i++)
    {
        ...
        if (ost->ist)
        {
            ...
            if (ost->st->disposition & AV_DISPOSITION_DEFAULT) have_default[ost->type + 1] = 1;
        }
    }
    ...
    else
    {
        ...
        for (int i = 0; i < ctx->nb_streams; i++)
        {
            ...
            if (nb_streams[type + 1] < 2 || have_default[type + 1] || ost->st->disposition & AV_DISPOSITION_ATTACHED_PIC) continue;
...
const char *const forced_keyframes_const_names[] = { "n", "n_forced", "prev_forced_n", "prev_forced_t", "t", NULL };
...
static int parse_forced_key_frames(void *log, KeyframeForceCtx *kf, const Muxer *mux, const char *spec)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (strstr(p, "chapters") == p)
        {
            ...
            if (nb_ch > INT_MAX - size || !(pts = av_realloc_f(pts, size += nb_ch - 1, sizeof(*pts)))) return AVERROR(ENOMEM);
...
static int process_forced_keyframes(Muxer *mux, const OptionsContext *o)
{
    for (int i = 0; i < mux->of.nb_streams; i++)
    {
        ...
        if (!(ost->type == AVMEDIA_TYPE_VIDEO && ost->enc_ctx && forced_keyframes)) continue;

        if (!strncmp(forced_keyframes, "expr:", 5))
        {
            int ret = av_expr_parse(&ost->kf.pexpr, forced_keyframes + 5, forced_keyframes_const_names, NULL, NULL, NULL, NULL, 0, NULL);
            if (ret < 0)
            {
...
static int validate_enc_avopt(Muxer *mux, const AVDictionary *codec_avopt)
{
    ...
    while ((e = av_dict_iterate(unused_opts, e)))
    {
        ...
        if (!strcmp(e->key, "gop_timecode")) continue;
...
int of_open(const OptionsContext *o, const char *filename)
{
    ...
    if (strcmp(oc->oformat->name, "rtp")) want_sdp = 0;

    of->format = oc->oformat;
    if (recording_time != INT64_MAX) oc->duration = recording_time;
    ...
    {
        of->bitexact = check_opt_bitexact(oc, mux->opts, "fflags", AVFMT_FLAG_BITEXACT);
    }
    ...
    if (!(oc->oformat->flags & AVFMT_NOFILE))
    {
        ...
        /* open the file */
        if ((err = avio_open2(&oc->pb, filename, AVIO_FLAG_WRITE, &oc->interrupt_callback, &mux->opts)) < 0)
        {
