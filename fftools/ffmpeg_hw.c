...
HWDevice *hw_device_get_by_name(const char *name)
{
    ...
    for (i = 0; i < nb_hw_devices; i++)
    {
        if (!strcmp(hw_devices[i]->name, name)) return hw_devices[i];
    }
    return NULL;
}
...
static HWDevice *hw_device_add(void)
{
    ...
    if (!hw_devices[nb_hw_devices]) return NULL;
    ...
    else if (nb_hw_devices > 0)
    {
        ...
        if (nb_hw_devices > 1)
            av_log(NULL, AV_LOG_WARNING,
                ...
                nb_hw_devices, dev->name, av_hwdevice_get_type_name(dev->type), dev->name);
