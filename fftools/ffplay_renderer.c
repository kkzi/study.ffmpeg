...
static void vk_log_cb(void *log_priv, enum pl_log_level level, const char *msg)
{
    ...
    if (level > 0 && level < FF_ARRAY_ELEMS(level_map)) av_log(log_priv, level_map[level], "%s\n", msg);
}
...
static int add_device_extension(const AVDictionary *opt, AVDictionary **dict)
{
    ...
    if (entry && entry->value && entry->value[0]) av_bprintf(&buf, "+%s", entry->value);
...
static int create_vk_by_hwcontext(VkRenderer *renderer, const char **ext, unsigned num_ext, const AVDictionary *opt)
{
    ...
    if (!ctx->placebo_vulkan) return AVERROR_EXTERNAL;
...
static int get_decode_queue(VkRenderer *renderer, int *index, int *count)
{
    ...
    *count = 0;
    ...
    if (!num_queue_family_prop) return AVERROR_EXTERNAL;

    queue_family_prop = av_calloc(num_queue_family_prop, sizeof(*queue_family_prop));
    if (!queue_family_prop) return AVERROR(ENOMEM);
...
static int create_hw_frame(VkRenderer *renderer, AVFrame *frame)
{
    ...
    if (ctx->hw_frame_ref)
    {
        ...
        if (hw_frame->width == frame->width && hw_frame->height == frame->height && hw_frame->sw_format == src_hw_frame->sw_format) return 0;
    ...
    if (!ctx->constraints)
    {
        ...
        if (!ctx->constraints) return AVERROR(ENOMEM);
    }
    ...
    if (ctx->constraints->valid_sw_formats)
    {
        ...
        {
            if (*sw_formats == src_hw_frame->sw_format) break;
            sw_formats++;
        }
        if (*sw_formats == AV_PIX_FMT_NONE) return 0;
    }

    ctx->hw_frame_ref = av_hwframe_ctx_alloc(ctx->hw_device_ref);
    if (!ctx->hw_frame_ref) return AVERROR(ENOMEM);
...
{
    if (!ctx->hw_frame_ref || !ctx->transfer_formats) return 0;

    for (int i = 0; ctx->transfer_formats[i] != AV_PIX_FMT_NONE; i++)
        if (ctx->transfer_formats[i] == frame->format) return 1;
...
static int map_frame(VkRenderer *renderer, AVFrame *frame, int use_hw_frame)
{
    ...
    if (use_hw_frame && !ctx->hw_frame_ref) return AVERROR(ENOSYS);
    ...
    if (!ret) return move_to_output_frame(ctx, frame);

    if (ret != AVERROR(ENOSYS)) av_log(NULL, AV_LOG_FATAL, "Map frame failed: %s\n", av_err2str(ret));
    return ret;
}
...
static int transfer_frame(VkRenderer *renderer, AVFrame *frame, int use_hw_frame)
{
    ...
    if (use_hw_frame && !check_hw_transfer(ctx, frame)) return AVERROR(ENOSYS);

    av_frame_unref(ctx->vk_frame);
    if (use_hw_frame) av_hwframe_get_buffer(ctx->hw_frame_ref, ctx->vk_frame, 0);
    ret = av_hwframe_transfer_data(ctx->vk_frame, frame, 1);
    if (!ret) return move_to_output_frame(ctx, frame);

    if (ret != AVERROR(ENOSYS)) av_log(NULL, AV_LOG_FATAL, "Transfer frame failed: %s\n", av_err2str(ret));
    return ret;
}
...
static int convert_frame(VkRenderer *renderer, AVFrame *frame)
{
    ...
    if (frame->format == AV_PIX_FMT_VULKAN) return 0;
    ...
    for (int use_hw = 1; use_hw >= 0; use_hw--)
    {
        ...
        if (!ret) return 0;
        if (ret != AVERROR(ENOSYS)) return ret;
...
static int resize(VkRenderer *renderer, int width, int height)
{
    ...
    if (!pl_swapchain_resize(ctx->swapchain, &width, &height)) return AVERROR_EXTERNAL;
    return 0;
}
