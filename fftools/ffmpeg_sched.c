...
static int waiter_wait(Scheduler *sch, SchWaiter *w)
{
    ...
    if (!atomic_load(&w->choked)) return 0;
...
static int queue_alloc(ThreadQueue **ptq, unsigned nb_streams, unsigned queue_size, enum QueueType type)
{
    ...
    op = (type == QUEUE_PACKETS) ? objpool_alloc_packets() : objpool_alloc_frames();
    if (!op) return AVERROR(ENOMEM);

    tq = tq_alloc(nb_streams, queue_size, op, (type == QUEUE_PACKETS) ? pkt_move : frame_move);
    if (!tq)
    {
        objpool_free(&op);
...
static int task_stop(SchTask *task)
{
    ...
    if (!task->thread_running) return 0;
...
static int64_t trailing_dts(const Scheduler *sch, int count_finished)
{
    ...
    for (unsigned i = 0; i < sch->nb_mux; i++)
    {
        ...
        for (unsigned j = 0; j < mux->nb_streams; j++)
        {
            ...
            if (ms->source_finished && !count_finished) continue;
            if (ms->last_dts == AV_NOPTS_VALUE) return AV_NOPTS_VALUE;
...
int sch_stop(Scheduler *sch, int64_t *finish_ts)
{
    ...
    if (finish_ts) *finish_ts = trailing_dts(sch, 1);
...
int sch_add_mux_stream(Scheduler *sch, unsigned mux_idx)
{
    ...
    if (!ms->pre_mux_queue.fifo) return AVERROR(ENOMEM);
...
int sch_add_demux(Scheduler *sch, SchThreadFunc func, void *ctx)
{
    ...
    if (!d->send_pkt) return AVERROR(ENOMEM);
...
int sch_add_dec(Scheduler *sch, SchThreadFunc func, void *ctx, int send_end_ts)
{
    ...
    if (!dec->send_frame) return AVERROR(ENOMEM);
...
int sch_add_enc(Scheduler *sch, SchThreadFunc func, void *ctx, int (*open_cb)(void *opaque, const AVFrame *frame))
{
    ...
    if (!enc->send_pkt) return AVERROR(ENOMEM);
...
int sch_add_filtergraph(Scheduler *sch, unsigned nb_inputs, unsigned nb_outputs, SchThreadFunc func, void *ctx)
{
    ...
    if (nb_inputs)
    {
        ...
        if (!fg->inputs) return AVERROR(ENOMEM);
    ...
    if (nb_outputs)
    {
        ...
        if (!fg->outputs) return AVERROR(ENOMEM);
...
int sch_add_sq_enc(Scheduler *sch, uint64_t buf_size_us, void *logctx)
{
    ...
    if (!sq->sq) return AVERROR(ENOMEM);

    sq->frame = av_frame_alloc();
    if (!sq->frame) return AVERROR(ENOMEM);
...
int sch_sq_add_enc(Scheduler *sch, unsigned sq_idx, unsigned enc_idx, int limiting, uint64_t max_frames)
{
    ...
    if (max_frames != INT64_MAX) sq_limit_frames(sq->sq, enc->sq_idx[1], max_frames);
...
int sch_connect(Scheduler *sch, SchedulerNode src, SchedulerNode dst)
{
    ...
    case SCH_NODE_TYPE_DEMUX: {
        ...
        av_assert0(src.idx < sch->nb_demux && src.idx_stream < sch->demux[src.idx].nb_streams);
        ...
        case SCH_NODE_TYPE_MUX: {
            ...
            av_assert0(dst.idx < sch->nb_mux && dst.idx_stream < sch->mux[dst.idx].nb_streams);
    ...
    case SCH_NODE_TYPE_DEC: {
        ...
        case SCH_NODE_TYPE_FILTER_IN: {
            ...
            av_assert0(dst.idx < sch->nb_filters && dst.idx_stream < sch->filters[dst.idx].nb_inputs);
    ...
    case SCH_NODE_TYPE_FILTER_OUT: {
        ...
        av_assert0(src.idx < sch->nb_filters && src.idx_stream < sch->filters[src.idx].nb_outputs);
        // filtered frames go to encoding
        av_assert0(dst.type == SCH_NODE_TYPE_ENC && dst.idx < sch->nb_enc);
...
static int mux_task_start(SchMux *mux)
{
    ...
    for (unsigned i = 0; i < mux->nb_streams; i++)
    {
        ...
        while (av_fifo_read(ms->pre_mux_queue.fifo, &pkt, 1) >= 0)
        {
            if (pkt)
            {
                if (!ms->init_eof) ret = tq_send(mux->queue, i, pkt);
                av_packet_free(&pkt);
                if (ret == AVERROR_EOF)
                    ms->init_eof = 1;
                else if (ret < 0)
...
static int mux_init(Scheduler *sch, SchMux *mux)
{
    ...
    {
        if (sch->nb_mux_ready < sch->nb_mux) return 0;
...
int sch_mux_stream_ready(Scheduler *sch, unsigned mux_idx, unsigned stream_idx)
{
    ...
    if (++mux->nb_streams_ready == mux->nb_streams && sch->transcode_started) ret = mux_init(sch, mux);
...
int sch_mux_sub_heartbeat_add(Scheduler *sch, unsigned mux_idx, unsigned stream_idx, unsigned dec_idx)
{
    ...
    if (!mux->sub_heartbeat_pkt)
    {
        ...
        if (!mux->sub_heartbeat_pkt) return AVERROR(ENOMEM);
    }
...
static void schedule_update_locked(Scheduler *sch)
{
    ...
    if (atomic_load(&sch->terminate)) return;
    ...
    for (unsigned i = 0; i < sch->nb_mux; i++)
    {
        ...
        for (unsigned j = 0; j < mux->nb_streams; j++)
        {
            ...
            if (ms->source_finished) continue;
            if (dts == AV_NOPTS_VALUE && ms->last_dts != AV_NOPTS_VALUE) continue;
            if (dts != AV_NOPTS_VALUE && ms->last_dts - dts >= SCHEDULE_TOLERANCE) continue;
    ...
    for (unsigned type = 0; type < 2; type++)
        ...
        {
            SchWaiter *w = type ? &sch->filters[i].waiter : &sch->demux[i].waiter;
            if (w->choked_prev != w->choked_next) waiter_set(w, w->choked_next);
        }
}
...
static int check_acyclic_for_output(const Scheduler *sch, SchedulerNode src, uint8_t *filters_visited, SchedulerNode *filters_stack)
{
    ...
    while (1)
    {
        ...
        if (src.idx_stream < fg->nb_inputs)
        {
            ...
            if (fi->src_sched.type == SCH_NODE_TYPE_DEMUX) continue;

            // otherwise connected to another filtergraph
            av_assert0(fi->src_sched.type == SCH_NODE_TYPE_FILTER_OUT);

            // found a cycle
            if (filters_visited[fi->src_sched.idx] == CYCLE_NODE_STARTED) return AVERROR(EINVAL);
...
static int check_acyclic(Scheduler *sch)
{
    ...
    for (unsigned i = 0; i < sch->nb_mux; i++)
    {
        ...
        for (unsigned j = 0; j < mux->nb_streams; j++)
        {
            ...
            if (src.type != SCH_NODE_TYPE_FILTER_OUT) continue;
            src.idx_stream = 0;
...
static int start_prepare(Scheduler *sch)
{
    ...
    for (unsigned i = 0; i < sch->nb_demux; i++)
    {
        ...
        for (unsigned j = 0; j < d->nb_streams; j++)
        {
            ...
            if (!ds->nb_dst)
            {
                av_log(d, AV_LOG_ERROR, "Demuxer stream %u not connected to any sink\n", j);
                return AVERROR(EINVAL);
            }

            ds->dst_finished = av_calloc(ds->nb_dst, sizeof(*ds->dst_finished));
            if (!ds->dst_finished) return AVERROR(ENOMEM);
        }
    }
    ...
    for (unsigned i = 0; i < sch->nb_dec; i++)
    {
        ...
        if (!dec->src.type)
        {
            av_log(dec, AV_LOG_ERROR, "Decoder not connected to a source\n");
            return AVERROR(EINVAL);
        }
        if (!dec->nb_dst)
        {
            av_log(dec, AV_LOG_ERROR, "Decoder not connected to any sink\n");
            return AVERROR(EINVAL);
        }

        dec->dst_finished = av_calloc(dec->nb_dst, sizeof(*dec->dst_finished));
        if (!dec->dst_finished) return AVERROR(ENOMEM);
    }
    ...
    for (unsigned i = 0; i < sch->nb_enc; i++)
    {
        ...
        if (!enc->src.type)
        {
            av_log(enc, AV_LOG_ERROR, "Encoder not connected to a source\n");
            return AVERROR(EINVAL);
        }
        if (!enc->nb_dst)
        {
            av_log(enc, AV_LOG_ERROR, "Encoder not connected to any sink\n");
            return AVERROR(EINVAL);
        }

        enc->dst_finished = av_calloc(enc->nb_dst, sizeof(*enc->dst_finished));
        if (!enc->dst_finished) return AVERROR(ENOMEM);
    }
    ...
    for (unsigned i = 0; i < sch->nb_mux; i++)
    {
        ...
        for (unsigned j = 0; j < mux->nb_streams; j++)
        {
            ...
            case SCH_NODE_TYPE_DEMUX:
                ...
                break;
            default:
                av_log(mux, AV_LOG_ERROR, "Muxer stream #%u not connected to a source\n", j);
                return AVERROR(EINVAL);
            }
        }

        ret = queue_alloc(&mux->queue, mux->nb_streams, mux->queue_size, QUEUE_PACKETS);
        if (ret < 0) return ret;
    }
    ...
    for (unsigned i = 0; i < sch->nb_filters; i++)
    {
        ...
        for (unsigned j = 0; j < fg->nb_inputs; j++)
        {
            ...
            if (!fi->src.type)
            {
                av_log(fg, AV_LOG_ERROR, "Filtergraph input %u not connected to a source\n", j);
                return AVERROR(EINVAL);
            }
            ...
            {
            case SCH_NODE_TYPE_DEMUX:
                fi->src_sched = dec->src;
                break;
            case SCH_NODE_TYPE_ENC:
                fi->src_sched = sch->enc[dec->src.idx].src;
                break;
            default:
                av_assert0(0);
            }
        ...
        for (unsigned j = 0; j < fg->nb_outputs; j++)
        {
            ...
            if (!fo->dst.type)
            {
                av_log(fg, AV_LOG_ERROR, "Filtergraph %u output %u not connected to a sink\n", i, j);
                return AVERROR(EINVAL);
            }
        }
    }
...
int sch_wait(Scheduler *sch, uint64_t timeout_us, int64_t *transcode_ts)
{
    ...
    {
        struct timespec tv = { .tv_sec = timeout_us / 1000000, .tv_nsec = (timeout_us % 1000000) * 1000 };
...
static int send_to_enc_thread(Scheduler *sch, SchEnc *enc, AVFrame *frame)
{
    ...
    if (enc->in_finished) return AVERROR_EOF;

    ret = tq_send(enc->queue, 0, frame);
    if (ret < 0) enc->in_finished = 1;
...
static int send_to_enc_sq(Scheduler *sch, SchEnc *enc, AVFrame *frame)
{
    ...
    if (!frame)
    {
        for (unsigned i = 0; i < enc->nb_dst; i++)
        {
            ...
            if (enc->dst[i].type != SCH_NODE_TYPE_MUX) continue;
...
static int send_to_enc(Scheduler *sch, SchEnc *enc, AVFrame *frame)
{
    ...
    return (enc->sq_idx[0] >= 0) ? send_to_enc_sq(sch, enc, frame) : send_to_enc_thread(sch, enc, frame);
}
...
static int mux_queue_packet(SchMux *mux, SchMuxStream *ms, AVPacket *pkt)
{
    ...
    if (!av_fifo_can_write(q->fifo))
    {
        ...
        if (new_size <= packets)
        {
            av_log(mux, AV_LOG_ERROR, "Too many packets buffered for output stream.\n");
    ...
    if (pkt)
    {
        ...
        if (!tmp_pkt) return AVERROR(ENOMEM);
...
static int send_to_mux(Scheduler *sch, SchMux *mux, unsigned stream_idx, AVPacket *pkt)
{
    ...
    int64_t dts = (pkt && pkt->dts != AV_NOPTS_VALUE) ? av_rescale_q(pkt->dts + pkt->duration, pkt->time_base, AV_TIME_BASE_Q) : AV_NOPTS_VALUE;
    ...
    if (pkt)
    {
        ...
        if (ms->init_eof) return AVERROR_EOF;
...
static int demux_stream_send_to_dst(Scheduler *sch, const SchedulerNode dst, uint8_t *dst_finished, AVPacket *pkt, unsigned flags)
{
    ...
    if (*dst_finished) return AVERROR_EOF;

    if (pkt && dst.type == SCH_NODE_TYPE_MUX && (flags & DEMUX_SEND_STREAMCOPY_EOF))
    {
        av_packet_unref(pkt);
        pkt = NULL;
    }
    ...
    ret = (dst.type == SCH_NODE_TYPE_MUX) ? send_to_mux(sch, &sch->mux[dst.idx], dst.idx_stream, pkt) : tq_send(sch->dec[dst.idx].queue, 0, pkt);
    if (ret == AVERROR_EOF) goto finish;
...
static int demux_send_for_stream(Scheduler *sch, SchDemux *d, SchDemuxStream *ds, AVPacket *pkt, unsigned flags)
{
    ...
    for (unsigned i = 0; i < ds->nb_dst; i++)
    {
        ...
        if (to_send) av_packet_unref(to_send);
        if (ret == AVERROR_EOF)
            nb_done++;
        else if (ret < 0)
            return ret;
...
static int demux_flush(Scheduler *sch, SchDemux *d, AVPacket *pkt)
{
    ...
    for (unsigned i = 0; i < d->nb_streams; i++)
    {
        ...
        for (unsigned j = 0; j < ds->nb_dst; j++)
        {
            ...
            if (ds->dst_finished[j] || dst->type != SCH_NODE_TYPE_DEC) continue;
            ...
            if (dec->queue_end_ts)
            {
                ...
                if (max_end_ts.ts == AV_NOPTS_VALUE || (ts.ts != AV_NOPTS_VALUE && av_compare_ts(max_end_ts.ts, max_end_ts.tb, ts.ts, ts.tb) < 0))
                    max_end_ts = ts;
            }
        }
    }
...
int sch_demux_send(Scheduler *sch, unsigned demux_idx, AVPacket *pkt, unsigned flags)
{
    ...
    if (terminate) return AVERROR_EXIT;

    // flush the downstreams after seek
    if (pkt->stream_index == -1) return demux_flush(sch, d, pkt);
...
static int demux_done(Scheduler *sch, unsigned demux_idx)
{
    ...
    for (unsigned i = 0; i < d->nb_streams; i++)
    {
        ...
        if (err != AVERROR_EOF) ret = err_merge(ret, err);
    }
...
int sch_dec_receive(Scheduler *sch, unsigned dec_idx, AVPacket *pkt)
{
    ...
    if (ret >= 0 && !pkt->data && !pkt->side_data_elems && dec->queue_end_ts) dec->expect_end_ts = 1;
...
{
    if (frame) return tq_send(fg->queue, in_idx, frame);
    ...
    if (!fg->inputs[in_idx].send_finished)
    {
        ...
        if (atomic_fetch_add(&fg->nb_inputs_finished_send, 1) == fg->nb_inputs - 1) tq_send_finish(fg->queue, fg->nb_inputs);
    }
    return 0;
}
...
static int dec_send_to_dst(Scheduler *sch, const SchedulerNode dst, uint8_t *dst_finished, AVFrame *frame)
{
    ...
    if (!frame) goto finish;

    ret = (dst.type == SCH_NODE_TYPE_FILTER_IN) ? send_to_filter(sch, &sch->filters[dst.idx], dst.idx_stream, frame)
...
int sch_dec_send(Scheduler *sch, unsigned dec_idx, AVFrame *frame)
{
    ...
    for (unsigned i = 0; i < dec->nb_dst; i++)
    {
        ...
        if (i < dec->nb_dst - 1)
        {
            ...
            ret = frame->buf[0] ? av_frame_ref(to_send, frame) : av_frame_copy_props(to_send, frame);
            if (ret < 0) return ret;
        }
...
static int dec_done(Scheduler *sch, unsigned dec_idx)
{
    ...
    if (dec->queue_end_ts) av_thread_message_queue_set_err_recv(dec->queue_end_ts, AVERROR_EOF);

    for (unsigned i = 0; i < dec->nb_dst; i++)
    {
        int err = dec_send_to_dst(sch, dec->dst[i], &dec->dst_finished[i], NULL);
        if (err < 0 && err != AVERROR_EOF) ret = err_merge(ret, err);
    }
...
void sch_filter_receive_finish(Scheduler *sch, unsigned fg_idx, unsigned in_idx)
{
    ...
    if (!fi->receive_finished)
    {
        ...
        // close the control stream when all actual inputs are done
        if (++fg->nb_inputs_finished_receive == fg->nb_inputs) tq_receive_finish(fg->queue, fg->nb_inputs);
    }
}
...
static void *task_wrapper(void *arg)
{
    ...
    if (ret < 0) av_log(task->func_arg, AV_LOG_ERROR, "Task finished with error code: %d (%s)\n", ret, av_err2str(ret));

    switch (task->node.type)
    {
    case SCH_NODE_TYPE_DEMUX:
        err = demux_done(sch, task->node.idx);
        break;
    case SCH_NODE_TYPE_MUX:
        err = mux_done(sch, task->node.idx);
        break;
    case SCH_NODE_TYPE_DEC:
        err = dec_done(sch, task->node.idx);
        break;
    case SCH_NODE_TYPE_ENC:
        err = enc_done(sch, task->node.idx);
        break;
    case SCH_NODE_TYPE_FILTER_IN:
        err = filter_done(sch, task->node.idx);
        break;
    default:
        av_assert0(0);
    }
    ...
    if (ret < 0) atomic_store(&sch->task_failed, 1);

    av_log(
        task->func_arg, ret < 0 ? AV_LOG_ERROR : AV_LOG_VERBOSE, "Terminating thread with return code %d (%s)\n", ret, ret < 0 ? av_err2str(ret) : "success");
