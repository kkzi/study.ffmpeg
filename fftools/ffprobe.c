...
static const AVOption writer_options[] = { { "string_validation", "set string validation mode", OFFSET(string_validation), AV_OPT_TYPE_INT,
                                               { .i64 = WRITER_STRING_VALIDATION_REPLACE }, 0, WRITER_STRING_VALIDATION_NB - 1, .unit = "sv" },
    { "sv", "set string validation mode", OFFSET(string_validation), AV_OPT_TYPE_INT, { .i64 = WRITER_STRING_VALIDATION_REPLACE }, 0,
        WRITER_STRING_VALIDATION_NB - 1, .unit = "sv" },
    { "ignore", NULL, 0, AV_OPT_TYPE_CONST, { .i64 = WRITER_STRING_VALIDATION_IGNORE }, .unit = "sv" },
    { "replace", NULL, 0, AV_OPT_TYPE_CONST, { .i64 = WRITER_STRING_VALIDATION_REPLACE }, .unit = "sv" },
    { "fail", NULL, 0, AV_OPT_TYPE_CONST, { .i64 = WRITER_STRING_VALIDATION_FAIL }, .unit = "sv" },
    { "string_validation_replacement", "set string validation replacement string", OFFSET(string_validation_replacement), AV_OPT_TYPE_STRING, { .str = "" } },
    { "svr", "set string validation replacement string", OFFSET(string_validation_replacement), AV_OPT_TYPE_STRING, { .str = "\xEF\xBF\xBD" } }, { NULL } };
        ...
        while (*p)
        {
            ...
            if (ret < 0)
            {
                ...
                bprint_bytes(&bp, p0, p - p0), av_log(wctx, AV_LOG_ERROR, "Invalid UTF8 sequence %s found in string validation replace '%s'\n", bp.str,
                                                   (*wctx)->string_validation_replacement);
                return ret;
            }
        }
    }
...
static inline void writer_print_section_header(WriterContext *wctx, void *data, int section_id)
{
    ...
    if (wctx->writer->print_section_header) wctx->writer->print_section_header(wctx, data);
}
    ...
    for (p = (uint8_t *)src; *p;)
    {
        ...
        if (invalid)
        {
            ...
            case WRITER_STRING_VALIDATION_FAIL:
                av_log(wctx, AV_LOG_ERROR, "Invalid UTF-8 sequence found in string '%s'\n", src);
...
static inline int writer_print_string(WriterContext *wctx, const char *key, const char *val, int flags)
{
    ...
    if (show_optional_fields == SHOW_OPTIONAL_FIELDS_NEVER ||
        (show_optional_fields == SHOW_OPTIONAL_FIELDS_AUTO && (flags & PRINT_STRING_OPT) && !(wctx->writer->flags & WRITER_FLAG_DISPLAY_OPTIONAL_FIELDS)))
        return 0;
...
static const Writer *writer_get_by_name(const char *name)
{
    ...
    for (i = 0; registered_writers[i]; i++)
        if (!strcmp(registered_writers[i]->name, name)) return registered_writers[i];
...
static void default_print_section_header(WriterContext *wctx, void *data)
{
    ...
    if (parent_section && !(parent_section->flags & (SECTION_FLAG_IS_WRAPPER | SECTION_FLAG_IS_ARRAY)))
    {
        ...
        av_bprintf(&wctx->section_pbuf[wctx->level], "%s%s:", wctx->section_pbuf[wctx->level - 1].str,
            upcase_string(buf, sizeof(buf), av_x_if_null(section->element_name, section->name)));
    }

    if (def->noprint_wrappers || def->nested_section[wctx->level]) return;

    if (!(section->flags & (SECTION_FLAG_IS_WRAPPER | SECTION_FLAG_IS_ARRAY))) writer_printf(wctx, "[%s]\n", upcase_string(buf, sizeof(buf), section->name));
}
...
static void default_print_str(WriterContext *wctx, const char *key, const char *value)
{
    ...
    if (!def->nokey) writer_printf(wctx, "%s%s=", wctx->section_pbuf[wctx->level].str, key);
...
static const char *c_escape_str(AVBPrint *dst, const char *src, const char sep, void *log_ctx)
{
    ...
    for (p = src; *p; p++)
    {
        ...
        case '\\':
            ...
            break;
        default:
            if (*p == sep) av_bprint_chars(dst, '\\', 1);
...
static void compact_print_section_header(WriterContext *wctx, void *data)
{
    ...
    av_bprint_clear(&wctx->section_pbuf[wctx->level]);
    if (parent_section && (section->flags & SECTION_FLAG_HAS_TYPE ||
                              (!(section->flags & SECTION_FLAG_IS_ARRAY) && !(parent_section->flags & (SECTION_FLAG_IS_WRAPPER | SECTION_FLAG_IS_ARRAY)))))
    {
        ...
        av_bprintf(section_pbuf, "%s%s", wctx->section_pbuf[wctx->level - 1].str, element_name);
        ...
        if (section->flags & SECTION_FLAG_HAS_TYPE)
        {
            ...
            {
                char c = (*p >= '0' && *p <= '9') || (*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z') ? av_tolower(*p) : '_';
    ...
    {
        if (parent_section && !(parent_section->flags & (SECTION_FLAG_IS_WRAPPER | SECTION_FLAG_IS_ARRAY)) && wctx->level && wctx->nb_item[wctx->level - 1])
...
static void compact_print_section_footer(WriterContext *wctx)
{
    ...
    if (!compact->nested_section[wctx->level] && compact->terminate_line[wctx->level] &&
...
static void compact_print_str(WriterContext *wctx, const char *key, const char *value)
{
    ...
    if (!compact->nokey) writer_printf(wctx, "%s%s=", wctx->section_pbuf[wctx->level].str, key);
...
static void ini_print_section_header(WriterContext *wctx, void *data)
{
    ...
    if (wctx->nb_item[wctx->level - 1]) writer_w8(wctx, '\n');
    ...
    if (!(section->flags & (SECTION_FLAG_IS_ARRAY | SECTION_FLAG_IS_WRAPPER))) writer_printf(wctx, "[%s]\n", buf->str);
}
...
static const AVOption json_options[] = { { "compact", "enable compact output", OFFSET(compact), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1 },
    { "c", "enable compact output", OFFSET(compact), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1 }, { NULL } };
...
static void json_print_section_header(WriterContext *wctx, void *data)
{
    ...
    if (wctx->level && wctx->nb_item[wctx->level - 1]) writer_put_str(wctx, ",\n");
...
static void json_print_str(WriterContext *wctx, const char *key, const char *value)
{
    ...
    if (wctx->nb_item[wctx->level] || (parent_section && parent_section->id == SECTION_ID_PACKETS_AND_FRAMES)) writer_put_str(wctx, json->item_sep);
    if (!json->compact) JSON_INDENT();
...
static void xml_print_section_header(WriterContext *wctx, void *data)
{
    ...
    if (section->flags & (SECTION_FLAG_IS_ARRAY | SECTION_FLAG_HAS_VARIABLE_FIELDS))
    {
        ...
        if (section->flags & SECTION_FLAG_HAS_TYPE)
        {
            AVBPrint buf;
            av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
            av_bprint_escape(&buf, section->get_type(data), NULL, AV_ESCAPE_MODE_XML, AV_ESCAPE_FLAG_XML_DOUBLE_QUOTES);
...
static void xml_print_value(WriterContext *wctx, const char *key, const void *value, const int is_int)
{
    ...
    if (section->flags & SECTION_FLAG_HAS_VARIABLE_FIELDS)
    {
        xml->indent_level++;
        XML_INDENT();
        av_bprint_escape(&buf, key, NULL, AV_ESCAPE_MODE_XML, AV_ESCAPE_FLAG_XML_DOUBLE_QUOTES);
        ...
        {
            av_bprint_escape(&buf, (const char *)value, NULL, AV_ESCAPE_MODE_XML, AV_ESCAPE_FLAG_XML_DOUBLE_QUOTES);
    ...
    {
        if (wctx->nb_item[wctx->level]) writer_w8(wctx, ' ');
...
#define print_list_fmt(k, f, n, m, ...)                                                                                                                        \
    ...
    {                                                                                                                                                          \
        ...
        {                                                                                                                                                      \
            ...
            {                                                                                                                                                  \
                if (idx > 0 || idx2 > 0) av_bprint_chars(&pbuf, ' ', 1);                                                                                       \
                av_bprintf(&pbuf, f, __VA_ARGS__);                                                                                                             \
            }                                                                                                                                                  \
        }                                                                                                                                                      \
        writer_print_string(w, k, pbuf.str, 0);                                                                                                                \
...
static void print_dovi_metadata(WriterContext *w, const AVDOVIMetadata *dovi)
{
...

    {
        ...
        print_int("chroma_resampling_explicit_filter_flag", hdr->chroma_resampling_explicit_filter_flag);
        ...
        print_int("spatial_resampling_filter_flag", hdr->spatial_resampling_filter_flag);
        print_int("el_spatial_resampling_filter_flag", hdr->el_spatial_resampling_filter_flag);
        ...
        for (int c = 0; c < 3; c++)
        {
            ...
            for (int i = 0; i < curve->num_pivots - 1; i++)
            {
                ...
                case AV_DOVI_MAPPING_POLYNOMIAL:
                    ...
                    print_list_fmt("poly_coef", "%" PRIi64, curve->poly_order[i] + 1, 1, curve->poly_coef[i][idx]);
                    break;
                case AV_DOVI_MAPPING_MMR:
                    print_str("mapping_idc_name", "mmr");
                    print_int("mmr_order", curve->mmr_order[i]);
                    print_int("mmr_constant", curve->mmr_constant[i]);
                    print_list_fmt("mmr_coef", "%" PRIi64, curve->mmr_order[i], 7, curve->mmr_coef[i][idx][idx2]);
                    break;
                default:
        ...
        print_list_fmt(
            "ycc_to_rgb_matrix", "%d/%d", FF_ARRAY_ELEMS(color->ycc_to_rgb_matrix), 1, color->ycc_to_rgb_matrix[idx].num, color->ycc_to_rgb_matrix[idx].den);
        print_list_fmt(
            "ycc_to_rgb_offset", "%d/%d", FF_ARRAY_ELEMS(color->ycc_to_rgb_offset), 1, color->ycc_to_rgb_offset[idx].num, color->ycc_to_rgb_offset[idx].den);
        print_list_fmt(
            "rgb_to_lms_matrix", "%d/%d", FF_ARRAY_ELEMS(color->rgb_to_lms_matrix), 1, color->rgb_to_lms_matrix[idx].num, color->rgb_to_lms_matrix[idx].den);
...
static void print_dynamic_hdr10_plus(WriterContext *w, const AVDynamicHDRPlus *metadata)
{
    ...
    for (int n = 1; n < metadata->num_windows; n++)
    {
        ...
        print_int("semimajor_axis_internal_ellipse", params->semimajor_axis_internal_ellipse);
        print_int("semimajor_axis_external_ellipse", params->semimajor_axis_external_ellipse);
        print_int("semiminor_axis_external_ellipse", params->semiminor_axis_external_ellipse);
    ...
    {
        print_int("num_rows_targeted_system_display_actual_peak_luminance", metadata->num_rows_targeted_system_display_actual_peak_luminance);
        ...
        for (int i = 0; i < metadata->num_rows_targeted_system_display_actual_peak_luminance; i++)
        {
            ...
            {
                print_q("targeted_system_display_actual_peak_luminance", metadata->targeted_system_display_actual_peak_luminance[i][j], '/');
            }
        }
    }
    for (int n = 0; n < metadata->num_windows; n++)
    {
        ...
        print_int("num_distribution_maxrgb_percentiles", params->num_distribution_maxrgb_percentiles);
        for (int i = 0; i < params->num_distribution_maxrgb_percentiles; i++)
        {
            print_int("distribution_maxrgb_percentage", params->distribution_maxrgb[i].percentage);
            print_q("distribution_maxrgb_percentile", params->distribution_maxrgb[i].percentile, '/');
        }
        print_q("fraction_bright_pixels", params->fraction_bright_pixels, '/');
    }
    if (metadata->mastering_display_actual_peak_luminance_flag)
    {
        print_int("num_rows_mastering_display_actual_peak_luminance", metadata->num_rows_mastering_display_actual_peak_luminance);
        print_int("num_cols_mastering_display_actual_peak_luminance", metadata->num_cols_mastering_display_actual_peak_luminance);
        for (int i = 0; i < metadata->num_rows_mastering_display_actual_peak_luminance; i++)
        {
            ...
            {
                print_q("mastering_display_actual_peak_luminance", metadata->mastering_display_actual_peak_luminance[i][j], '/');
            }
        }
    }
    ...
    for (int n = 0; n < metadata->num_windows; n++)
    {
        ...
        if (params->tone_mapping_mode_flag)
        {
            ...
            for (int i = 0; i < params->tone_mapping_param_num; i++)
            {
                ...
                print_q("targeted_system_display_maximum_luminance", tm_params->targeted_system_display_maximum_luminance, '/');
                ...
                if (tm_params->base_enable_flag)
                {
                    ...
                    print_int("base_param_Delta_enable_mode", tm_params->base_param_Delta_enable_mode);
                ...
                if (tm_params->three_Spline_enable_flag)
                {
                    ...
                    for (int j = 0; j < tm_params->three_Spline_num; j++)
                    {
                        ...
                        if (three_spline->th_mode == 0 || three_spline->th_mode == 2) print_q("3Spline_TH_enable_MB", three_spline->th_enable_mb, '/');
...
static void print_pkt_side_data(WriterContext *w, AVCodecParameters *par, const AVPacketSideData *sd, SectionID id_data)
{
    ...
    {
        if (do_show_data) writer_print_data(w, "data", sd->data, sd->size);
...
static void show_packet(WriterContext *w, InputFile *ifile, AVPacket *pkt, int packet_idx)
{
    ...
    print_fmt("flags", "%c%c%c", pkt->flags & AV_PKT_FLAG_KEY ? 'K' : '_', pkt->flags & AV_PKT_FLAG_DISCARD ? 'D' : '_',
    ...
    if (pkt->side_data_elems)
    {
        ...
        {
            print_pkt_side_data(w, st->codecpar, &pkt->side_data[i], SECTION_ID_PACKET_SIDE_DATA);
            writer_print_section_footer(w);
        }
        writer_print_section_footer(w);
    }
...
static void show_frame(WriterContext *w, AVFrame *frame, AVStream *stream, AVFormatContext *fmt_ctx)
{
...
#if LIBAVUTIL_VERSION_MAJOR < 59
        AV_NOWARN_DEPRECATED(print_int("coded_picture_number", frame->coded_picture_number);
    ...
    if (frame->nb_side_data) print_frame_side_data(w, frame, stream);
...
static av_always_inline int process_frame(WriterContext *w, InputFile *ifile, AVFrame *frame, const AVPacket *pkt, int *packet_new)
{
    ...
    if (dec_ctx)
    {
        ...
        case AVMEDIA_TYPE_SUBTITLE:
            if (*packet_new) ret = avcodec_decode_subtitle2(dec_ctx, &sub, &got_frame, pkt);
            *packet_new = 0;
            break;
        default:
            *packet_new = 0;
...
static int read_interval_packets(WriterContext *w, InputFile *ifile, const ReadInterval *interval, int64_t *cur_ts)
{
    ...
    while (!av_read_frame(fmt_ctx, pkt))
    {
        ...
        if (selected_streams[pkt->stream_index])
        {
            ...
            if (pts != AV_NOPTS_VALUE) *cur_ts = av_rescale_q(pts, tb, AV_TIME_BASE_Q);
    ...
    for (i = 0; i < ifile->nb_streams; i++)
    {
        ...
        if (do_read_frames)
        {
            while (process_frame(w, ifile, frame, pkt, &(int){ 1 }) > 0)
                ;
            if (ifile->streams[i].dec_ctx) avcodec_flush_buffers(ifile->streams[i].dec_ctx);
        }
    }
...
static int show_stream(WriterContext *w, AVFormatContext *fmt_ctx, int stream_idx, InputStream *ist, int in_program)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (dec_ctx) print_int("refs", dec_ctx->refs);
        break;
    ...
    {
        if (dec_ctx && dec_ctx->codec->priv_class) print_private_data(w, dec_ctx->priv_data);
        if (fmt_ctx->iformat->priv_class) print_private_data(w, fmt_ctx->priv_data);
    }
...
#define PRINT_DISPOSITION(flagname, name)                                                                                                                      \
    ...
    if (stream->codecpar->nb_coded_side_data)
    {
        ...
        {
            print_pkt_side_data(w, stream->codecpar, &stream->codecpar->coded_side_data[i], SECTION_ID_STREAM_SIDE_DATA);
            writer_print_section_footer(w);
        }
        writer_print_section_footer(w);
    }
...
static int open_input_file(InputFile *ifile, const char *filename, const char *print_filename)
{
    ...
    for (i = 0; i < fmt_ctx->nb_streams; i++)
    {
        ...
        }
        {
            ...
            err = filter_codec_opts(codec_opts, stream->codecpar->codec_id, fmt_ctx, stream, codec, &opts);
            if (err < 0) exit(1);
...
#define CHECK_END                                                                                                                                              \
    ...
    {
        if (do_show_frames && do_show_packets && wctx->writer->flags & WRITER_FLAG_PUT_PACKETS_AND_FRAMES_IN_SAME_CHAPTER)
            section_id = SECTION_ID_PACKETS_AND_FRAMES;
        else if (do_show_packets && !do_show_frames)
            section_id = SECTION_ID_PACKETS;
        else  // (!do_show_packets && do_show_frames)
            section_id = SECTION_ID_FRAMES;
        if (do_show_frames || do_show_packets) writer_print_section_header(wctx, NULL, section_id);
    ...
    if (ifile.fmt_ctx) close_input_file(&ifile);
...
static int opt_show_optional_fields(void *optctx, const char *opt, const char *arg)
{
    ...
    {
        double num;
        int ret = parse_number("show_optional_fields", arg, OPT_INT, SHOW_OPTIONAL_FIELDS_AUTO, SHOW_OPTIONAL_FIELDS_ALWAYS, &num);
        if (ret < 0) return ret;
...
static int opt_output_file_o(void *optctx, const char *opt, const char *arg)
{
    if (output_filename)
    {
        av_log(NULL, AV_LOG_ERROR, "Argument '%s' provided as output filename, but '%s' was already specified.\n", arg, output_filename);
        return AVERROR(EINVAL);
    }
    if (!strcmp(arg, "-")) arg = "fd:";
    output_filename = arg;
...
static void print_section(SectionID id, int level)
{
    ...
    printf("%c%c%c%c", section->flags & SECTION_FLAG_IS_WRAPPER ? 'W' : '.', section->flags & SECTION_FLAG_IS_ARRAY ? 'A' : '.',
        section->flags & SECTION_FLAG_HAS_VARIABLE_FIELDS ? 'V' : '.', section->flags & SECTION_FLAG_HAS_TYPE ? 'T' : '.');
...
static inline int check_section_show_entries(int section_id)
{
    ...
    for (const SectionID *id = section->children_ids; *id != -1; id++)
        if (check_section_show_entries(*id)) return 1;
    return 0;
}
...
#define SET_DO_SHOW(id, varname)                                                                                                                               \
    do                                                                                                                                                         \
    {                                                                                                                                                          \
        if (check_section_show_entries(SECTION_ID_##id)) do_show_##varname = 1;                                                                                \
    } while (0)
...
int main(int argc, char **argv)
{
    ...
    if (!output_format) output_format = av_strdup("default");
    if (!output_format)
    {
        ret = AVERROR(ENOMEM);
        goto end;
    ...
    if ((ret = writer_open(&wctx, w, w_args, sections, FF_ARRAY_ELEMS(sections), output_filename)) >= 0)
    {
        ...
        if (!input_filename && ((do_show_format || do_show_programs || do_show_streams || do_show_chapters || do_show_packets || do_show_error) ||
