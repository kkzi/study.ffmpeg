    ...
    {                                                                                                                                                          \
        ...
        {                                                                                                                                                      \
            ...
            av_log(NULL, level, "%slib%-11s %2d.%3d.%3d / %2d.%3d.%3d\n", indent, #libname, LIB##LIBNAME##_VERSION_MAJOR, LIB##LIBNAME##_VERSION_MINOR,        \
                LIB##LIBNAME##_VERSION_MICRO, AV_VERSION_MAJOR(version), AV_VERSION_MINOR(version), AV_VERSION_MICRO(version));                                \
        }                                                                                                                                                      \
...
static void print_program_info(int flags, int level)
{
    ...
    if (flags & SHOW_COPYRIGHT) av_log(NULL, level, " Copyright (c) %d-%d the FFmpeg developers", program_birth_year, CONFIG_THIS_YEAR);
    av_log(NULL, level, "\n");
...
static void print_codec(const AVCodec *c)
{
    ...
    if (c->capabilities & AV_CODEC_CAP_DRAW_HORIZ_BAND) printf("horizband ");
    if (c->capabilities & AV_CODEC_CAP_DR1) printf("dr1 ");
    if (c->capabilities & AV_CODEC_CAP_DELAY) printf("delay ");
    if (c->capabilities & AV_CODEC_CAP_SMALL_LAST_FRAME) printf("small ");
    if (c->capabilities & AV_CODEC_CAP_EXPERIMENTAL) printf("exp ");
    if (c->capabilities & AV_CODEC_CAP_CHANNEL_CONF) printf("chconf ");
    if (c->capabilities & AV_CODEC_CAP_PARAM_CHANGE) printf("paramchange ");
    if (c->capabilities & AV_CODEC_CAP_VARIABLE_FRAME_SIZE) printf("variable ");
    if (c->capabilities & (AV_CODEC_CAP_FRAME_THREADS | AV_CODEC_CAP_SLICE_THREADS | AV_CODEC_CAP_OTHER_THREADS)) printf("threads ");
    if (c->capabilities & AV_CODEC_CAP_AVOID_PROBING) printf("avoidprobe ");
    if (c->capabilities & AV_CODEC_CAP_HARDWARE) printf("hardware ");
    if (c->capabilities & AV_CODEC_CAP_HYBRID) printf("hybrid ");
    if (!c->capabilities) printf("none");
    printf("\n");

    if (c->type == AVMEDIA_TYPE_VIDEO || c->type == AVMEDIA_TYPE_AUDIO)
    {
        printf("    Threading capabilities: ");
        switch (c->capabilities & (AV_CODEC_CAP_FRAME_THREADS | AV_CODEC_CAP_SLICE_THREADS | AV_CODEC_CAP_OTHER_THREADS))
        {
...
static const AVCodec *next_codec_for_id(enum AVCodecID id, void **iter, int encoder)
{
    ...
    {
        if (c->id == id && (encoder ? av_codec_is_encoder(c) : av_codec_is_decoder(c))) return c;
    }
    return NULL;
}
...
static void show_help_filter(const char *name)
{
    ...
    for (i = 0; i < count; i++)
    {
        printf("       #%d: %s (%s)\n", i, avfilter_pad_get_name(f->inputs, i), av_get_media_type_string(avfilter_pad_get_type(f->inputs, i)));
    }
    ...
    for (i = 0; i < count; i++)
    {
        printf("       #%d: %s (%s)\n", i, avfilter_pad_get_name(f->outputs, i), av_get_media_type_string(avfilter_pad_get_type(f->outputs, i)));
    }
    ...
    if (f->priv_class) show_help_children(f->priv_class, AV_OPT_FLAG_VIDEO_PARAM | AV_OPT_FLAG_FILTERING_PARAM | AV_OPT_FLAG_AUDIO_PARAM);
    if (f->flags & AVFILTER_FLAG_SUPPORT_TIMELINE) printf("This filter has support for timeline through the 'enable' option.\n");
    #else
    av_log(NULL, AV_LOG_ERROR,
...
int show_help(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!topic) return AVERROR(ENOMEM);
    par = strchr(topic, '=');
    if (par) *par++ = 0;
...
static int get_codecs_sorted(const AVCodecDescriptor ***rcodecs)
{
    ...
    while ((desc = avcodec_descriptor_next(desc)))
        nb_codecs++;
    if (!(codecs = av_calloc(nb_codecs, sizeof(*codecs)))) return AVERROR(ENOMEM);
    desc = NULL;
    while ((desc = avcodec_descriptor_next(desc)))
        codecs[i++] = desc;
...
int show_codecs(void *optctx, const char *opt, const char *arg)
{
    ...
    if (nb_codecs < 0) return nb_codecs;
    ...
    for (i = 0; i < nb_codecs; i++)
    {
        ...
        printf(" %c%c%c%c%c%c", avcodec_find_decoder(desc->id) ? 'D' : '.', avcodec_find_encoder(desc->id) ? 'E' : '.', get_media_type_char(desc->type),
            (desc->props & AV_CODEC_PROP_INTRA_ONLY) ? 'I' : '.', (desc->props & AV_CODEC_PROP_LOSSY) ? 'L' : '.',
...
static void print_codecs(int encoder)
{
    ...
    for (i = 0; i < nb_codecs; i++)
    {
        ...
        while ((codec = next_codec_for_id(desc->id, &iter, encoder)))
        {
            printf(" %c%c%c%c%c%c", get_media_type_char(desc->type), (codec->capabilities & AV_CODEC_CAP_FRAME_THREADS) ? 'F' : '.',
                (codec->capabilities & AV_CODEC_CAP_SLICE_THREADS) ? 'S' : '.', (codec->capabilities & AV_CODEC_CAP_EXPERIMENTAL) ? 'X' : '.',
                (codec->capabilities & AV_CODEC_CAP_DRAW_HORIZ_BAND) ? 'B' : '.', (codec->capabilities & AV_CODEC_CAP_DR1) ? 'D' : '.');
...
int show_filters(void *optctx, const char *opt, const char *arg)
{
    ...
    while ((filter = av_filter_iterate(&opaque)))
    {
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            if (!j)
                *(descr_cur++) = ((!i && (filter->flags & AVFILTER_FLAG_DYNAMIC_INPUTS)) || (i && (filter->flags & AVFILTER_FLAG_DYNAMIC_OUTPUTS))) ? 'N' : '|';
        }
        *descr_cur = 0;
        printf(" %c%c%c %-17s %-10s %s\n", filter->flags & AVFILTER_FLAG_SUPPORT_TIMELINE ? 'T' : '.', filter->flags & AVFILTER_FLAG_SLICE_THREADS ? 'S' : '.',
...
int show_pix_fmts(void *optctx, const char *opt, const char *arg)
{
    ...
    while ((pix_desc = av_pix_fmt_desc_next(pix_desc)))
    {
        ...
        printf("%c%c%c%c%c %-16s       %d            %3d      %d", sws_isSupportedInput(pix_fmt) ? 'I' : '.', sws_isSupportedOutput(pix_fmt) ? 'O' : '.',
            pix_desc->flags & AV_PIX_FMT_FLAG_HWACCEL ? 'H' : '.', pix_desc->flags & AV_PIX_FMT_FLAG_PAL ? 'P' : '.',
            pix_desc->flags & AV_PIX_FMT_FLAG_BITSTREAM ? 'B' : '.', pix_desc->name, pix_desc->nb_components, av_get_bits_per_pixel(pix_desc),
...
int init_report(const char *env, FILE **file)
{
    ...
    while (env && *env)
    {
        ...
        {
            if (count) av_log(NULL, AV_LOG_ERROR, "Failed to parse FFREPORT environment variable: %s\n", av_err2str(ret));
            break;
        }
        if (*env) env++;
        count++;
    ...
    av_log(NULL, AV_LOG_INFO,
        ...
        "Report written to \"%s\"\n"
        "Log level: %d\n",
        program_name, tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec, filename.str, report_file_level);
...
static void print_device_list(const AVDeviceInfoList *device_list)
{
    ...
    for (int i = 0; i < device_list->nb_devices; i++)
    {
        ...
        printf("%c %s [%s] (", device_list->default_device == i ? '*' : ' ', device->device_name, device->device_description);
...
static int print_device_sources(const AVInputFormat *fmt, AVDictionary *opts)
{
    ...
    if (!fmt || !fmt->priv_class || !AV_IS_INPUT_DEVICE(fmt->priv_class->category)) return AVERROR(EINVAL);
...
static int print_device_sinks(const AVOutputFormat *fmt, AVDictionary *opts)
{
    ...
    if (!fmt || !fmt->priv_class || !AV_IS_OUTPUT_DEVICE(fmt->priv_class->category)) return AVERROR(EINVAL);
