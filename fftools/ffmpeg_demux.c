...
InputStream *ist_find_unused(enum AVMediaType type)
{
    ...
    {
        if (ist->par->codec_type == type && ist->discard && ist->user_set_discard != AVDISCARD_ALL) return ist;
    }
    return NULL;
}
...
static void report_new_stream(Demuxer *d, const AVPacket *pkt)
{
    ...
    if (pkt->stream_index < d->nb_streams_warn) return;
...
static void ifile_duration_update(Demuxer *d, DemuxStream *ds, int64_t last_duration)
{
    ...
    if (ds->max_pts > ds->min_pts && ds->max_pts - (uint64_t)ds->min_pts < INT64_MAX - last_duration) last_duration += ds->max_pts - ds->min_pts;

    if (!d->duration || av_compare_ts(d->duration, d->time_base, last_duration, ds->ist.st->time_base) < 0)
    {
...
static void ts_discontinuity_detect(Demuxer *d, InputStream *ist, AVPacket *pkt)
{
    ...
    if (copy_ts && ds->next_dts != AV_NOPTS_VALUE && fmt_is_discont && ist->st->pts_wrap_bits < 60)
    {
        ...
        if (FFABS(wrap_dts - ds->next_dts) < FFABS(pkt_dts - ds->next_dts) / 10) disable_discontinuity_correction = 0;
    }
    ...
    if (ds->next_dts != AV_NOPTS_VALUE && !disable_discontinuity_correction)
    {
        ...
        if (fmt_is_discont)
        {
            if (FFABS(delta) > 1LL * dts_delta_threshold * AV_TIME_BASE || pkt_dts + AV_TIME_BASE / 10 < ds->dts)
            {
                ...
                if (pkt->pts != AV_NOPTS_VALUE) pkt->pts -= av_rescale_q(delta, AV_TIME_BASE_Q, pkt->time_base);
            }
        }
        else
        {
...
static int ist_dts_update(DemuxStream *ds, AVPacket *pkt)
{
    ...
    if (ds->next_dts == AV_NOPTS_VALUE) ds->next_dts = ds->dts;

    if (pkt->dts != AV_NOPTS_VALUE) ds->next_dts = ds->dts = av_rescale_q(pkt->dts, pkt->time_base, AV_TIME_BASE_Q);
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        else if (ist->par->framerate.num != 0)
        {
            ...
            if (ist->codec_desc && (ist->codec_desc->props & AV_CODEC_PROP_FIELDS) && av_stream_get_parser(ist->st))
    ...
    if (ds->streamcopy_needed)
    {
        ...
        if (!pkt->opaque_ref) return AVERROR(ENOMEM);
...
#define SHOW_TS_DEBUG(tag_)                                                                                                                                    \
    ...
    if (pkt->dts != AV_NOPTS_VALUE) pkt->dts += av_rescale_q(ifile->ts_offset, AV_TIME_BASE_Q, pkt->time_base);
    if (pkt->pts != AV_NOPTS_VALUE) pkt->pts += av_rescale_q(ifile->ts_offset, AV_TIME_BASE_Q, pkt->time_base);

    if (pkt->pts != AV_NOPTS_VALUE) pkt->pts *= ds->ts_scale;
    if (pkt->dts != AV_NOPTS_VALUE) pkt->dts *= ds->ts_scale;
    ...
    if (pkt->dts != AV_NOPTS_VALUE) pkt->dts += duration;
...
static int input_packet_process(Demuxer *d, DemuxMsg *msg, AVPacket *src)
{
    ...
    if (debug_ts)
    {
        av_log(NULL, AV_LOG_INFO,
            ...
            av_ts2str(input_files[ist->file_index]->ts_offset), av_ts2timestr(input_files[ist->file_index]->ts_offset, &AV_TIME_BASE_Q));
    }
...
static void readrate_sleep(Demuxer *d)
{
    ...
    for (int i = 0; i < f->nb_streams; i++)
    {
        ...
        if (pts - burst_until > now) av_usleep(pts - burst_until - now);
    }
}
...
static void *input_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        if (do_pkt_dump)
        {
            av_pkt_dump_log2(NULL, AV_LOG_INFO, pkt, do_hex_dump, f->ctx->streams[pkt->stream_index]);
        }

        /* the following test is needed in case new streams appear
           dynamically in stream : we ignore them */
        if (pkt->stream_index >= f->nb_streams || f->streams[pkt->stream_index]->discard)
        {
        ...
        if (f->readrate) readrate_sleep(d);
        ...
        if (ret < 0)
        {
            if (ret != AVERROR_EOF) av_log(f, AV_LOG_ERROR, "Unable to send packet to main thread: %s\n", av_err2str(ret));
...
static int thread_start(Demuxer *d)
{
    ...
    if (d->thread_queue_size <= 0) d->thread_queue_size = (nb_input_files > 1 ? 8 : 1);

    if (nb_input_files > 1 && (f->ctx->pb ? !f->ctx->pb->seekable : strcmp(f->ctx->iformat->name, "lavfi"))) d->non_blocking = 1;
    ret = av_thread_message_queue_alloc(&d->in_thread_queue, d->thread_queue_size, sizeof(DemuxMsg));
    if (ret < 0) return ret;
    ...
    if (d->loop)
    {
        ...
        {
            InputStream *ist = f->streams[i];
            nb_audio_dec += !!(ist->decoding_needed && ist->st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO);
        }

        if (nb_audio_dec)
        {
            ret = av_thread_message_queue_alloc(&f->audio_duration_queue, nb_audio_dec, sizeof(LastFrameDuration));
            if (ret < 0) goto fail;
...
int ifile_get_packet(InputFile *f, AVPacket **pkt)
{
    ...
    ret = av_thread_message_queue_recv(d->in_thread_queue, &msg, d->non_blocking ? AV_THREAD_MESSAGE_NONBLOCK : 0);
    if (ret < 0) return ret;
    if (msg.looping) return 1;
...
static void demux_final_stats(Demuxer *d)
{
    ...
    for (int j = 0; j < f->nb_streams; j++)
    {
        ...
        if (ist->discard || type == AVMEDIA_TYPE_ATTACHMENT) continue;
        ...
        {
            av_log(f, AV_LOG_VERBOSE, "%" PRIu64 " frames decoded; %" PRIu64 " decode errors", ist->frames_decoded, ist->decode_errors);
            if (type == AVMEDIA_TYPE_AUDIO) av_log(f, AV_LOG_VERBOSE, " (%" PRIu64 " samples)", ist->samples_decoded);
...
void ifile_close(InputFile **pf)
{
    ...
    if (d->read_started) demux_final_stats(d);
...

{
    ...
    if (codec_name)
    {
        ...
        if (recast_media && st->codecpar->codec_type != (*pcodec)->type) st->codecpar->codec_type = (*pcodec)->type;
        return 0;
    }
    else
    {
        if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO && hwaccel_id == HWACCEL_GENERIC && hwaccel_device_type != AV_HWDEVICE_TYPE_NONE)
        {
            ...
            while ((c = av_codec_iterate(&i)))
            {
                ...
                if (c->id != st->codecpar->codec_id || !av_codec_is_decoder(c)) continue;
...
static int guess_input_channel_layout(InputStream *ist, int guess_layout_max)
{
    ...
    if (dec->ch_layout.order == AV_CHANNEL_ORDER_UNSPEC)
    {
        ...
        if (dec->ch_layout.nb_channels > guess_layout_max) return 0;
        av_channel_layout_default(&dec->ch_layout, dec->ch_layout.nb_channels);
        if (dec->ch_layout.order == AV_CHANNEL_ORDER_UNSPEC) return 0;
...
static int add_display_matrix_to_stream(const OptionsContext *o, AVFormatContext *ctx, InputStream *ist)
{
    ...
    if (!rotation_set && !hflip_set && !vflip_set) return 0;
...
static int ist_add(const OptionsContext *o, Demuxer *d, AVStream *st)
{
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
    {
        ...
        if (hwaccel)
        {
            ...
            if (!strcmp(hwaccel, "nvdec") || !strcmp(hwaccel, "cuvid")) hwaccel = "cuda";
        ...
        if (hwaccel_device)
        {
            ...
            if (!ist->hwaccel_device) return AVERROR(ENOMEM);
        }
    }

    ret = choose_decoder(o, ic, st, ist->hwaccel_id, ist->hwaccel_device_type, &ist->dec);
    if (ret < 0) return ret;

    ret = filter_codec_opts(o->g->codec_opts, ist->st->codecpar->codec_id, ic, st, ist->dec, &ist->decoder_opts);
    if (ret < 0) return ret;
    ...
    if (!ist->dec_ctx) return AVERROR(ENOMEM);

    if (discard_str)
    {
        const AVOption *discard_opt = av_opt_find(ist->dec_ctx, "skip_frame", NULL, 0, 0);
    ...
    if (o->bitexact) ist->dec_ctx->flags |= AV_CODEC_FLAG_BITEXACT;
    ...
    case AVMEDIA_TYPE_SUBTITLE: {
        ...
        if (canvas_size)
        {
            ret = av_parse_video_size(&ist->dec_ctx->width, &ist->dec_ctx->height, canvas_size);
            if (ret < 0)
            {
    ...
    if (!ist->par) return AVERROR(ENOMEM);
...
static int dump_attachment(InputStream *ist, const char *filename)
{
    ...
    if (!*filename && (e = av_dict_get(st->metadata, "filename", NULL, 0))) filename = e->value;
    if (!*filename)
    {
    ...
    if (ret >= 0) av_log(ist, AV_LOG_INFO, "Wrote attachment (%d bytes) to '%s'\n", st->codecpar->extradata_size, filename);
...
int ifile_open(const OptionsContext *o, const char *filename)
{
    ...
    if (!strcmp(filename, "-")) filename = "fd:";
    ...
    if (o->nb_frame_pix_fmts) av_dict_set(&o->g->format_opts, "pixel_format", o->frame_pix_fmts[o->nb_frame_pix_fmts - 1].u.str, 0);
    ...
    if (video_codec_name) ret = err_merge(ret, find_codec(NULL, video_codec_name, AVMEDIA_TYPE_VIDEO, 0, &ic->video_codec));
    if (audio_codec_name) ret = err_merge(ret, find_codec(NULL, audio_codec_name, AVMEDIA_TYPE_AUDIO, 0, &ic->audio_codec));
    if (subtitle_codec_name) ret = err_merge(ret, find_codec(NULL, subtitle_codec_name, AVMEDIA_TYPE_SUBTITLE, 0, &ic->subtitle_codec));
    if (data_codec_name) ret = err_merge(ret, find_codec(NULL, data_codec_name, AVMEDIA_TYPE_DATA, 0, &ic->data_codec));
    if (ret < 0)
    {
    ...
    if (o->bitexact) ic->flags |= AVFMT_FLAG_BITEXACT;
    ...
    if (err < 0)
    {
        ...
        if (err == AVERROR_PROTOCOL_NOT_FOUND) av_log(d, AV_LOG_ERROR, "Did you mean file:%s?\n", filename);
        return err;
    }
    f->ctx = ic;
    ...
    if (!o->seek_timestamp && ic->start_time != AV_NOPTS_VALUE) timestamp += ic->start_time;
