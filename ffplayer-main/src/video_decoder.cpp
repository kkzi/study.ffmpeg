...
void VideoDecoder::decode_loop()
{
    ...
    for (;;)
    {
        ...
        {
            spdlog::warn("the serial in video packet queue and decoder is different, serial={}, serial={}", m_ctx->video_packet_queue.serial(), m_pkt_serial);
            break;
        }
    }
    av_frame_free(&frame);
...
bool VideoDecoder::drop_frame(AVFrame *frame)
{
    ...
    if (framedrop > 0 || (framedrop && m_ctx->master_clock->sync_type() != SYNC_TYPE_VIDEO) && frame->pts != AV_NOPTS_VALUE)
    {
        ...
        if (!isnan(diff) && fabs(diff) < AV_NOSYNC_THRESHOLD && diff - m_ctx->frame_last_filter_delay < 0 && m_pkt_serial == m_ctx->video_clock.serial() &&
...
bool VideoDecoder::enqueue_frame(AVFrame *frame)
{
    ...
    vp->pts = frame->pts;
    vp->duration = frame->pkt_duration;
